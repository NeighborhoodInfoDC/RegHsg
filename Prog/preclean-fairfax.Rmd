...
There are `r parcel_temp %>% group_by(parcel_id) %>% filter(n()>1) %>% nrow()` parcels that show up more than once on the parcel file. These may be unit counts but it is unclear. 
...
There are `r dwelling_temp %>% group_by(parcel_id) %>% filter(n()>1) %>% nrow()` parcels that show up more than once on the parcel file.
...
There are `r land_temp %>% group_by(parcel_id) %>% filter(n()>1) %>% nrow()` parcels that show up more than once in the land file. For now, for each parcel that has multiple entries, I take the sum of the area and the first value of all of the other values. Can clean up later if we want to use this data
