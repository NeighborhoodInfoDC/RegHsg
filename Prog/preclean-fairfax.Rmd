...
```{r}
tax_temp <- left_join(parcel_final, dwelling_final, by="parcel_id")
```
...
```{r}
tax_final <- left_join(tax_temp, land_final, by="parcel_id")
```


Merge onto Blacknight file

There are `r (!tax_final$parcel_id%in%jur_temp$parcel_id) %>% sum` units in the combined tax data that are not in the BlacKnight data.  
There are `r (!jur_temp$parcel_id%in% tax_final$parcel_id) %>% sum` units in the tax data that are not in the BlackKnight data.


```{r merge}
jur_final <- left_join(tax_final, jur_temp, by="parcel_id")
```
