...
NbClust::NbClust(t1, method = "kmeans", min.nc = 2, max.nc = 10)
...
Clean up groups and export.

```{r clean-groups}

clean_clusters <- t_clusters %>% 
  bind_rows(a_clusters) %>% 
...
write_csv(clean_clusters,
          "L:/Libraries/RegHsg/Prog/Tables/all-nodes-grouped_v2.csv")
...
t_clusters %>% 
  ...
  select(type, group = .cluster, name, 
         ...
         number_of_units, residential_acres,
         density_residential) %>% 
  write_csv("L:/Libraries/RegHsg/Prog/Tables/transit-clusters.csv")
...
Zoom in on a few regions to characterize the problem.
...
ggplot() +
  geom_sf(dcb, mapping = aes(),
          fill = "grey") +
  geom_sf(dcn, mapping = aes(fill = factor(.cluster)),
          alpha = .4) +
  coord_sf(datum = NA)
...
dcnd <- dcn %>% 
  group_by(.cluster) %>% 
  arrange(desc(.cluster), desc(density_residential)) %>% 
  st_difference()

ggplot() +
  geom_sf(dcb, mapping = aes(),
          fill = "grey") +
  geom_sf(dcnd, mapping = aes(fill = factor(.cluster))) +
  coord_sf(datum = NA)
...
Use the percentage of a buffer to calcalate.
...
t_weighted0$total_area <-st_area(t_weighted0) %>% units::set_units("acres") %>% set_units(NULL)


t_weighted <- t_weighted0 %>% 
  group_by(.cluster) %>% 
  arrange(desc(.cluster), density_residential) %>% 
  st_difference()

t_weighted$wt_area <-st_area(t_weighted) %>% units::set_units("acres") %>% set_units(NULL)

t_weighted <- t_weighted %>% 
  mutate(wt = wt_area / total_area,
         wt_units_min = wt * new_units_min,
         wt_units_mid = wt * new_units_midpoint)

```

Calculate sum of new units.

```{r sum-units}

t_weighted %>% 
  as.data.frame() %>% 
  summarize_at(vars(wt_units_min, wt_units_mid),
               sum)

```

Units created per node
...
ggplot() +
  geom_sf(bound, mapping = aes(),
          fill = "grey") +
  geom_sf(t_weighted, mapping = aes(fill = wt_units_min)) +
  coord_sf(datum = NA)
