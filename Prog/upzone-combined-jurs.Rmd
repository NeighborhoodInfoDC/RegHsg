...
Fill in single family homes that have single addresses with 1 unit.

```{r fill-sf}

parcel1 <- parcel %>% 
  mutate(numberofunits = ifelse(category == "sf" & address_type == "single",
                                1,
                                numberofunits))
...
Tabulate missing number of units.
...
parcel1 %>% 
  ...
  summarize(count = n(),
            ...
            p.na = units.na / n(),
            p.zero = units.zero / n()) %>% 
  knitr::kable(caption = "All residential, non-vacant properties")
...
parcel1 %>% 
  ...
  summarize(count = n(),
            ...
            p.na = units.na / n(),
            p.zero = units.zero / n()) %>% 
  knitr::kable(caption = "Multifamily properties")
  ...
  dataset %>% 
    ...
    summarize(residential_parcels = sum(residential, na.rm = TRUE),
              residential_acres = sum(ifelse(residential == 1, lotsize_acres, 0),
                                      na.rm = TRUE),
              total_acres = sum(lotsize_acres, na.rm = TRUE),
              number_of_units = sum(ifelse(residential == 1,
                                           numberofunits,
                                           0),na.rm = TRUE),
              density = number_of_units / total_acres) %>% 
