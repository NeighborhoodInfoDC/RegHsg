...
ss <- filter(nodes_varies, name == "Silver Spring", node == "Transit")
cc <- filter(nodes_varies, name == "Crystal City", node == "Transit")
gr <- filter(nodes_varies, name == "Grosvenor-Strathmore", node == "Transit")
...
bk_ss <- st_intersection(parcel2, ss)
bk_cc <- st_intersection(parcel2, cc)
bk_gr <- st_intersection(parcel2, gr)
    ...
    st_intersection(n) %>% 
        ...
        summarize(residential_parcels = sum(residential, na.rm = TRUE),
                  ...
                  pother = sum(ifelse(category == "other", lotsize_acres, 0),
                              na.rm = TRUE) / total_acres,
                  pmfres = sum(ifelse(category == "mf", lotsize_acres, 0),
                            na.rm = TRUE) / residential_acres) %>% 
...
get_individual_density(dataset = parcel2,
                       ...
                       node_name = "Crystal City", 
                       node_type = "Transit") %>% 
  knitr::kable()

ssd <- get_individual_density(dataset = parcel2,
                       node_geometry = nodes_uniform,
                       node_name = "Silver Spring", 
                       node_type = "Transit")

gsd <- get_individual_density(dataset = parcel2,
                       node_geometry = nodes_uniform,
                       node_name = "Grosvenor-Strathmore", 
                       node_type = "Transit")
...
st_area(clarendon) %>% units::set_units("acres") %>% set_units(NULL)
...
get_clusters <- function(dataset) {
  ...
  clusters <- kmeans(rescaled_data, centers = 6, nstart = 6)
  
  augment(clusters, dataset)
...
```{r cluster-t}
t_clusters <- get_clusters(individual_transit)
...
a_clusters <- get_clusters(individual_activity_varied)
...
write_csv(upzone_summary, "tables/upzoning-summary.csv")
...
t_clusters %>% bind_rows(a_clusters) %>% 
  ...
  select(type, group = .cluster, name, 
         ...
         density_residential) %>% 
  write_csv("tables/all-nodes-grouped.csv")
...
#### ARCHIVED: other groupings

##### distance

```{r dist-calc}

distance <- individual_varies %>% 
  # pick baseline nodes
  mutate(group1.dr = density_residential[name == "Clarksburg" & type == "Activity"],
         group2.dr = density_residential[name == "Clarendon" & type == "Activity"],
         group3.dr = density_residential[name == "Dupont" & type == "Activity"]) %>% 
  # calculate distance from each
  mutate(group1.dist.dr = (density_residential - group1.dr)^2,
         group2.dist.dr = (density_residential - group2.dr)^2,
         group3.dist.dr = (density_residential - group3.dr)^2) %>% 
  # get minimum density
  mutate(min.dist = pmin(group1.dist.dr,
                         group2.dist.dr,
                         group3.dist.dr)) %>% 
  # pick group based on minimum density
  mutate(.cluster = case_when(
    min.dist == group1.dist.dr ~ 1,
    min.dist == group2.dist.dr ~ 2,
    min.dist == group3.dist.dr ~ 3
  )) %>% 
...
man <- individual_varies %>% 
  mutate(.cluster = case_when(
    density_residential < 2 ~ "Outerlying suburb",
    density_residential >= 2 & density_residential < 5 ~ "Low density suburb",
    density_residential >= 5 & density_residential < 10 ~ "Mid density suburb",
    density_residential >= 10 & density_residential < 20 ~ "High density suburb",
    density_residential >= 20 & density_residential < 35 ~ "Low density urban area",
    density_residential >= 35 & density_residential < 60 ~ "Mid-density urban area",
    density_residential >= 60 & density_residential < 100 ~ "Dense urban area",
    density_residential >= 100 ~ "Urban core",
    TRUE ~ "error"
  )) %>% 
  mutate(.cluster = factor(.cluster, levels = c("Outerlying suburb",
                                                "Low density suburb",
                                                "Mid density suburb",
                                                "High density suburb",
