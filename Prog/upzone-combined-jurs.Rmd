...
parcel2 <- parcel1 %>%
  ...
  #                          "other",
  #                          category)) %>% 
         mutate(category = ifelse(is.na(category) & numberofunits > 1 & !is.na(numberofunits),
                                  "mf",
                                  category),
                category = ifelse(is.na(category) & numberofunits == 1 & !is.na(numberofunits),
                                  "sf",
                                  category),
                category = ifelse(is.na(category) & is.na(numberofunits),
                                  "other",
                                  category)) %>% 
...
parcel2 %>% 
  ...
  group_by(county_name, category) %>% 
  summarize(n(),
            numberofunits = sum(numberofunits),
...
### Use cluster analysis sort nodes

```{r cluster}

set.seed(20190322)

rescaled_data <- individual %>% 
  select(-name, -type) %>% 
  mutate(density_residential = ifelse(density_residential == "NaN",
                0,
                density_residential)) %>% 
  mutate_if(is.numeric, scales::rescale, to = c(1, 100))

clusters <- kmeans(rescaled_data, centers = 3, nstart = 3)

tidy_clusters <-augment(x = clusters, individual)

x <- tidy(clusters)

names(x) <- c(names(rescaled_data), "size", "withinss", "cluster")


data <- individual %>% 
  select(name, type, density_area, pmf)

rs1 <- individual %>% 
  select(density_area, pmf) %>% 
  mutate_if(is.numeric, scales::rescale, to = c(1, 100)) %>% 
  mutate(d1 = density_area,
         d2 = density_area)


clusters1 <- kmeans(rs1, centers = 3, nstart = 3)

tidy_clusters1 <-augment(x = clusters1, data)

x1 <- tidy(clusters1)

names(x1) <- c(names(rs1), "size", "withinss", "cluster")

map_clusters <- function(dataset) {
  
  dataset %>% 
    left_join(nodes, by = c("name", "type")) %>% 
    ggplot() +
...
tidy_clusters1 %>% 
  ggplot() +
  geom_point(mapping = aes(density_area, pmf, color = .cluster))
