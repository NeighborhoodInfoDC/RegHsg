...
clusters <- kmeans(rescaled_data, centers = 5, nstart = 5)

tidy_clusters <- augment(clusters, individual_varies)
...
distance <- individual_varies %>% 
  # pick baseline nodes
  mutate(group1.dr = density_residential[name == "Bethesda" & type == "Activity"],
...
values <- tidy_clusters %>% 
  group_by(.cluster) %>% 
  summarize(midpoint = min(density_residential) + 
                ((max(density_residential) - min(density_residential)) / 2 )) %>% 
  pull(midpoint)

upzone <- tidy_clusters %>% 
  mutate(midpoint2 = values[2],
         midpoint3 = values[3]) %>% 
  mutate(new_units = case_when(
    .cluster == 1 ~ midpoint2 * residential_acres,
    .cluster == 2 ~ midpoint3 * residential_acres,
    TRUE ~ 0
  )) %>% 
  mutate(units_added = ifelse(.cluster == 3,
                              0,
                              new_units - number_of_units))


upzone_summary <- upzone %>% 
  group_by(.cluster) %>% 
  summarize(units_added = sum(units_added))
