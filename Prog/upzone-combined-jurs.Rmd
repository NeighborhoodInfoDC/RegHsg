...
parcel <- parcel %>% 
  mutate(arc_lotsize_sf = lotsize_sf, # archive the original lot size variable
         lotsize_sf = ifelse(county_name == "Arlington" & is.na(lotsize_sf),
                             parcel_area,
                             lotsize_sf),
         lotsize_sf = ifelse(county_name == "Arlington" & is.na(lotsize_sf),
                             lotsize_prop,
...
points_all <- get_node_geography(node_type = "joint",
                                style = "point",
                                activity_buffer = 1,
                                transit_buffer = .5) %>% 
  mutate(node = ifelse(type == "Activity",
...
points <- st_join(points_all, bound, st_intersects, left = FALSE) 

nodes <- left_join(as.data.fr)
