...
places<-places("Virginia") %>% st_as_sf() %>% st_transform(st_crs(zon)) %>% filter(NAME=="Herndon"|NAME=="Vienna"|NAME=="Clifton") %>% select()
...
Of those that do not match, 92% (107,951) are in planning development districts, some of which have residential uses. However, developers must pass the development plan process before building in these districts. 35 of those classified as "residential weird" are vacant.
```{r results = F}
