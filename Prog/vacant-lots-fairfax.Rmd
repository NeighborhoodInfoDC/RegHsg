...
```{r}
tempjur <- jur_4 %>% mutate(newId = seq(1, nrow(jur_4), 1))

tempjur1<-tempjur %>% group_by(id) %>% count() %>% arrange(desc(n)) %>% filter(n>1)
...
sames1 <- differents %>% filter(newZoning==ZONECODE)
differents1 <- differents %>% filter(newZoning != ZONECODE)
...
differents1 %<>% mutate(ZONECODE = case_when(
                     ...
                     ZONECODE=="R16" ~ "R-16",
                     T ~ ZONECODE
)) %>% mutate(newZoning=case_when(newZoning=="PLANNED RES MIXED USE"~ZONECODE,
...
differents2 <- differents1 %>% filter(newZoning != ZONECODE)
...
Fairfax does not have dual zoningcodes.
...
tempTable %>% group_by(type) %>% summarise(totalLots = sum(n, na.rm=T))%>%
  knitr::kable()

```

Of those that do not match, 92% (107,951) are in planning development districts, some of which have residential uses. However, developers must pass the development plan process before building in these districts.  
```{r results = F}
...
zoningdata1 <- zoningdata %>% 
  ...
  mutate(lotsize_sf = as.numeric(lotsize_sf),
         lot_area_min = gsub(lot_area, pattern=",", replacement = "") %>% as.numeric) %>%
  filter(lotsize_sf>=lot_area_min|is.na(lot_area_min))
