...
```{r}
vacant1 %<>% mutate(newZoning=case_when(
...
paste0(sum(is.na(vacant2$ZONECODE)), " observations didn't join- ",
      round(sum(is.na(vacant2$ZONECODE)) / nrow(vacant2) * 100, 2), "%")

if (sum(is.na(vacant2$ZONECODE)) / nrow(vacant2) > .05) {
...
#lost 3 observations. May try to debug at a future point
```
...
```{r}
(vacantfinal$zoning_code%>%unique)[!(vacantfinal$zoning_code %>% unique %in% (zoningcode$code %>%unique))] %>% sort 
```

```{r}
(zoningcode$code%>%unique)[!(zoningcode$code %>% unique %in% (vacantfinal$zoning_code%>%unique))] 
```
...
zoningdata <- left_join(vacantfinal, zoningcode, by = c("zoning_code"="code"))
...
anti_join(vacantfinal, zoningcode, by = c("zoning_code"="code")) %>% 
...
zoningdata1 <- zoningdata %>% 
  ...
  ungroup() %>%
  mutate(lotsize_sf = lotsize_sf %>% as.numeric())
