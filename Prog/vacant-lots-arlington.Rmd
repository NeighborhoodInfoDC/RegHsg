...
```{r filter}
zoningdata_vars <- zoningdata1 %>% 
  mutate(buildable_area = lotsize_sf * lot_coverage_max,
         living_area_70 = buildable_area * .7,
         living_area_85 = buildable_area * .85,
         units_per_floor_70 = ifelse(dwelling_type == "One Family Dwelling",
                                     1,
                                     floor(living_area_70 / 900)),
         units_per_floor_85 = ifelse(dwelling_type == "One Family Dwelling",
                                     1,
                                     floor(living_area_85 / 900)),
         floors = floor(height_ft / 10),
         units_height_70 = units_per_floor_70 * floors,
         units_height_85 = units_per_floor_85 * floors,
         units_lotsize = floor(lotsize_sf / lotarea_per_unit)) %>% 
  mutate(total_units_70 = case_when(
    dwelling_type == "One Family Dwelling" ~ 1,
    units_height_70 > units_lotsize ~ units_lotsize,
    TRUE ~ units_height_70)) %>% 
  mutate(total_units_85 = case_when(
    dwelling_type == "One Family Dwelling" ~ 1,
    units_height_85 > units_lotsize ~ units_lotsize,
    TRUE ~ units_height_85))

```

Get summary statistics for the whole county

```{r sum}

zoningdata_vars %>% 
  summarize_at(vars(units_height_70, units_height_85,
                      units_lotsize, total_units_70, total_units_85),
               sum, na.rm = TRUE) %>% 
  knitr::kable()

zoningdata_vars %>% 
  filter(total_units_70 > 1) %>% 
  ggplot() +
  geom_point(mapping = aes(lotsize_sf, total_units_70)) +
