...
THe purpose of this program is to obtain additional data from the county itself to supplement the Black Knight data when possible.
...
There are `r parcel %>% group_by(parcel_id) %>% filter(n()>1)` parcels that show up more than once on the parcel file. If the parcels have an identical area, we combine them by taking the average area and width. 
...
newjur <- left_join(jur, parcel, by = c("assessorsparcelnumberapnpin" = "parcel_id"))
x <- anti_join(jur, parcel, by = c("assessorsparcelnumberapnpin" = "parcel_id"))
