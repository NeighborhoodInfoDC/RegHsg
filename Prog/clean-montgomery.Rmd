...
sfdetached <- c("FRATERNAL BUILDING",
                "FRATERNITY HOUSE",
                "GROUP HOME",
                "RECTORY",
                "RESIDENTIAL MOBILE HOME",
...
other <- c("FRATERNAL BUILDING",
        "FRATERNITY HOUSE","GROUP HOME",
        "HOME FOR THE ELDERLY",
        "MULTIPLE RESIDENCE")
...
filter(jur, is.na(lotsize_sf)) %>% 
  count(category)

filter(jur, is.na(buildingarea)) %>% 
  count(category)


jur <- jur %>% 
  mutate(lotsize_sf = if_else(is.na(lotsize_sf), parcel_area, lotsize_sf))
...
count( jur, is.na(parcel_area))
count(jur, is.na(lotsize_sf))
...
nested <- multiples %>% 
  ...
  summarize_at(vars(zoning,lotsize_sf,
                    ...
                    yearbuilt, long, lat, parcel_area, parcel_length), list) %>% 
...
Based on examining the 218 addressess with different criteria, we develop these assumptions:
...
### Spatial Join
Patrick's Notes. Filling in multifamily units with missing data that did not match on parcel number.
```{spatial join}



sf_contain(points, polygons)
sf_join()
