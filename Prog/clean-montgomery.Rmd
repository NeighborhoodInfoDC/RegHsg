...
Test parcel geometry
```{r test-geo}
st_geometry(parcel_sf)
parcel_sf <- st_transform(parcel_sf, crs = 4326)
parcel_sf <- parcel_sf %>% 
  mutate(area = st_area(geometry))


#ggplot() +
  #geom_sf(parcel_sf, mapping = aes())
```
...
parcel_sf <- parcel_sf %>% 
  ...
  mutate(parcel_id = paste0("00-",
                            substr(raw_parcelid_par, 1, 8))) %>% 
...
filter(jur, is.na(lotsize_acres)) %>% 
  count(category)
