...
rm(apartment, commercial, condo, duplex,
   office, sfdetached, trailer, townhouse,
...
```
##### `building_type`
Here, we expect NAs since we do not create a specific `building_type` for Loudon.
```{r bt-check, eval=F}
...
For townhouses and other forms of attached housing, we do not want a collapse, since this is a function of addressing and not the unit type. We will reclassify the address type as `single` for properties that have a value of `sf attached`(none in Loudoun) and `townhouse` for the `category_detail` variable.

```{r th}

jur <- jur %>% 
  mutate(address_type = ifelse(category_detail %in% c("townhouse"),
                                  "single",
                                  address_type))
...
nested <- multiples %>% 
  ...
  summarize_at(vars(zoning, lotsize_sf,
                    ...
                    yearbuilt, long, lat, parcel_area, parcel_length, lotsize_acres_prop, numberofunits_prop, propertytype_par, landusetype_prop), list) %>% 
...
There are no examples of this in Loudoun, but in each case we want to default to the more general case.
...
singles_f <- singles %>% 
  select(county_fips, county_name,
         ...
         category, category_detail, residential, building_type, parcel_area, parcel_length, lotsize_acres_prop, numberofunits_prop, propertytype_par, landusetype_prop)
