...
```{r read}
if (!exists("jur")) {
  jur <- readRDS("L:/Libraries/RegHsg/Data/montgomery/postcleaned-montgomery-data.Rdata")
} else {
  warning("Data already read in")
}
...
zoning_gaithersburg1 <- zoning_gaithersburg %>% 
  mutate(dwelling_type = str_trim(dwelling_type, side = "right"),
         dwelling_type = ifelse(dwelling_type == "single family semidetached", 
                                "single-family semidetached", 
                                dwelling_type),
         dwelling_type = ifelse(zoning_code == "R6",
                                "townhouse", 
                                dwelling_type),
         dwelling_type = ifelse(zone == "Mixed Use Development", 
                                "multiple family dwelling", 
                                dwelling_type),
         dwelling_type = ifelse(zone == "Central Business District", 
                                "multiple family dwelling", 
                                dwelling_type),
         dwelling_type = ifelse(zone == "Corridor Development", 
                                "multiple family dwelling", 
                                dwelling_type),
         dwelling_type = ifelse(zone == "RB", 
                                "townhouse", 
                                dwelling_type),
         flag = ifelse(dwelling_type == "single family semidetached", 
                       1, 
                       0))
...
zoning_gaithersburg2 <- zoning_gaithersburg1 %>% 
  mutate(dwelling_type = ifelse(is.na(dwelling_type) == 1, 
                                "all", 
                                dwelling_type)) %>% 
  mutate(dwelling_type = factor(dwelling_type,
        ...
        mutate(housing_type= case_when(zoning_code %in% SF_g ~ "SF", 
                                       ...
                                       zoning_code %in% COM_g ~ "MXD", 
                                       # categorize commercial to mixed use,
                                       # according to zoning ordinance
                                       zoning_code %in% MXD_g ~ "MXD")) %>% 
        mutate(lot_area_min = ifelse(housing_type == "MXD", 
                                     1800, 
                                     lot_area_min),
               height_ft = ifelse(housing_type == "MXD", 
                                  45, 
                                  height_ft),
               lot_coverage_max = ifelse(housing_type == "MXD",
                                         0.4, 
                                         lot_coverage_max),
               #assign MXD with R-20
               density= ifelse(housing_type == "MXD", 
                               21.5, 
                               density))

count(zoning_gaithersburg2, housing_type)

```

*For Rockville*

As the table shows, Rockville zoning code doesn't break out by the dwelling type we use for other jurisdictions.
...
zoning_rockville1 <- zoning_rockville %>% 
  mutate(newdwelling_type = ifelse(zone == "Mixed-Use Transition" | 
                                     dwelling_type == "Existing housing", 
                                   "all-2", 
                                   "all")) %>% 
  mutate(newdwelling_type = ifelse(is.na(newdwelling_type) == 1, 
                                   "all", 
                                   newdwelling_type)) %>% 
  ...
  mutate(housing_type = case_when (zoning_code %in%  sfcodes ~ "SF",
         ...
         lot_area_min = ifelse(housing_type=="Planned Development", 43560, lot_area_min),
         # following RMD-10 assumption
         height_ft = ifelse(housing_type == "Planned Development", 
                            40, 
                            height_ft),
         lot_coverage_max = ifelse(housing_type == "Planned Development", 
                                   0.3, 
                                   lot_coverage_max))
...
zoningtotal_rockvile <- left_join(total_rockvile, 
                                  zoning_rockville1, 
                                  by = "zoning_code")
zoningtotal_gaithersburg <- left_join(total_gaithersburg, 
                                      zoning_gaithersburg2, 
                                      by = "zoning_code")
...
zoningdata_rockvile1_total <- left_join(total_rockvile1, 
                                        zoning_rockville1, 
                                        by = "zoning_code")

zoningdata_gaithersburg1_total<- left_join(total_gaithersburg1, 
                                           zoning_gaithersburg2, 
                                           by = "zoning_code") %>% 
    mutate(one_unit_flag= ifelse(zoning_code == "R-A" | zoning_code == "R-90", 
                                 1, 
                                 0))
  ...
  get_unit_variables_r <- function(dataset, 
    ...
    dataset %>% 
      mutate(parameters = paste0("CE: ", construction_efficiency * 100,
             ...
             lot_coverage_max = ifelse(is.na(lot_coverage_max) == 1, 
                                       0.75, 
                                       lot_coverage_max),
      ...
      mutate(units_total = case_when(
        ...
        zoning_code=="RMD-15" & units_height>(15 *(lotsize_sf/43560)) ~ 
          floor(15  *  (lotsize_sf/43560)),
        zoning_code=="RMD-20" & units_height>(20 * (lotsize_sf/87120)) ~ 
          floor(20 * (lotsize_sf/87120)),
        zoning_code == "RMD-10" & units_height > (10 * (lotsize_sf/43560)) ~ 
          floor(10 * (lotsize_sf/43560)), #
        # RMD-10: 10 units per acre max
        housing_type == "Planned Development" & units_height > (10 * (lotsize_sf/43560))~ 
          floor(10 * (lotsize_sf/43560)),
        TRUE ~ units_height))
}
...
testdataset <- get_unit_variables_r(dataset = zoningdata_rockvile1_final,
  ...
  select(zoning_code, propaddress, dwelling_type, lotsize_sf, 
         height_ft, lot_coverage_max, units_height, buildable_area, 
         living_area, units_total, units_per_floor, floors, 
         one_unit_flag,  subset, lot_area_min, lotarea_per_unit, 
         smallsize_flag, housing_type, zone)
...
sumunits <- get_unit_summary_r(dataset =  Rockvillevacant,
                 construction_efficiency = .7,
                 average_unit_size = 940,
                 dwelling_type_breakdown = TRUE)

write_csv(sumunits,
          "L:/Libraries/RegHsg/Prog/Tables/montgomery-Rockville-vacant-lots-analysis.csv")
...
mf <- zoningdata_rockvile1_final %>% 
  filter(category == "vacant",
         housing_type %in% c("MF", "Planned Development")) %>% 
  group_by(propaddress) %>%
  count() %>% 
  nrow()

mf.summary <- 
  c(nrow(total_rockvile), 
    nrow(filter(total_rockvile, category == "vacant")), 
    mf, 
    sumunits %>% 
      filter(housing_type %in% c("MF", "Planned Development")) %>% 
      pull(lots) %>% 
      sum(),
    sumunits %>% 
      filter(housing_type %in% c("MF", "Planned Development")) %>% 
...
breakdown <- sumunits %>% 
  ungroup() %>% 
  mutate(housing_type = ifelse(housing_type %in% c("MF", "Planned Development"),
                                "Multifamily",
                                housing_type),
         housing_type = ifelse(housing_type %in% c("MF", "Planned Development"),
                                "Multifamily",
                                housing_type)) %>% 
  group_by(housing_type) %>% 
  summarize_at(vars(lots, units_total),sum) %>% 
  filter(housing_type != "All") %>% 
  arrange(desc(units_total))


mf.summary

breakdown %>% knitr::kable()
...
sumunitsg <- get_unit_summary_g(dataset =  gaithersburg1vacant,
                   construction_efficiency = .7,
                   average_unit_size = 940,
                   dwelling_type_breakdown = TRUE)
...
write_csv(sumunitsg,
          "L:/Libraries/RegHsg/Prog/Tables/montgomery-Gaithersburg-vacant-lots-analysis.csv")

```
Table for Geithersburg

```{r tableg}

mf <- zoningdata_gaithersburg1_final %>% 
  filter(category == "vacant",
         housing_type %in% c("MF", "MXD")) %>% 
  group_by(propaddress) %>%
  count() %>% 
  nrow()

mf.summary <- 
  c(nrow(total_gaithersburg), 
    nrow(filter(total_gaithersburg, category == "vacant")), 
    mf, 
    sumunitsg %>% 
      filter(housing_type %in% c("MF", "MXD")) %>% 
      pull(lots) %>% 
      sum(),
    sumunitsg %>% 
      filter(housing_type %in% c("MF", "MXD")) %>% 
...
breakdown <- sumunitsg %>% 
  ungroup() %>% 
  mutate(housing_type = ifelse(housing_type %in% c("MF", "MXD"),
                                "Multifamily",
                                housing_type),
         housing_type = ifelse(housing_type %in% c("MF", "MXD"),
                                "Multifamily",
                                housing_type)) %>% 
  group_by(housing_type) %>% 
  summarize_at(vars(lots, units_total),sum) %>% 
  filter(housing_type != "Sum") %>% 
  arrange(desc(units_total))
