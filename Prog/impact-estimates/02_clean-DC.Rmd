...
if (!file.exists(paste0("../../Data/", 
                        filepath, 
                        "-county-land-use-crosstab-new.csv"))) {
...
```{r}
## some False vacant are actually not vacant
jur %>% 
 filter(countylandusedescription == "FALSE VACANT") %>% 
  select(assessorsparcelnumberapnpin, parcelbase_address, owneroccupiedresidential,  parcelbase_phasebuild, parcelbase_newimpr) %>% 
    arrange(desc(parcelbase_phasebuild)) %>% 
  head(20)
...
jur <- jur %>% 
  mutate(category = case_when(
       #cast cemeteries, educational institutions, and parks, and allother as `all other category`
           parcelbase_mix1txtype %in% c("E2", "E7") | 
             parcelbase_usecode %in% allother | 
             nationalparkflag ==1 |
             parkflag ==1 ~ "all other category",
...
#reclassify any non-residential property as single except for office condos and commercial retail condo (one unit in residential condo used for retail)
#add 'not in' function
'%!in%' <- function(x,y)!('%in%'(x,y))

argh <- jur5 %>%
  mutate(address_type = ifelse((residential == 0 & parcelbase_usecode %!in% c("056","057","048"))
                               ,"single", address_type))
count(argh, address_type)
...
```
There are still parcels in the missing data set that could be collapsed based on parcelbase_address. Recreating Sarah's macro to do so.
...
classify_parceladdresses <- function(dataset) {
  ...
  dataset %>% 
    mutate(address_type = case_when(
      is.na(parcelbase_address) ~ "missing",
      parcelbase_address %in% singles ~ "single",
      parcelbase_address %in% multiples ~ "multiple"))
...
count(missing2, address_type)


#separate and recombine with previous single and multiple files.
...
allmultiple <- bind_rows(multiples, multiples2)

allsingle <- bind_rows(singles, singles2) 
...
nested <- allmultiple2 %>% 
  ...
  summarize_at(vars(zoning,lotsize_sf,
                    ...
                    parcelbase_phasebuild, parcelbase_phaseland, parcelbase_mix1txtype, parcelbase_newimpr), list) %>% 
  ...
  mutate(nprops = map(zoning_list, length),
         ...
         parcelbase_phasebuild=map_dbl(parcelbase_phasebuild_list,sum, na.rm = TRUE),
         parcelbase_phaseland=map_dbl(parcelbase_phaseland_list,sum, na.rm = TRUE),
         parcelbase_mix1txtype= map(parcelbase_mix1txtype_list, mode_narm),
         parcelbase_newimpr =map_dbl(parcelbase_newimpr_list,sum, na.rm = TRUE))%>% 
  mutate(ties_cat = ifelse(map(category, length) > 1, 1, 0)) %>% 
  mutate(category = ifelse(ties_cat == 1,
                           map(category_list, 1),
                           category)) 
...
**LH note 4/3/19**
No longer have any vacant properties in multiple - these were forced to single in earlier revisions to the code as we were incorrectly collapsing vacant properties where the BK address was incorrect - represented the mailing address. 
...
#YS comment: here I get a list of category
...
```{r singles-count}
count(allsingle, category_detail)
count(allsingle, category)
```
...
nrow(units) -
     (nrow(allsingle1) - nrow(anti_join(allsingle, units, by = c("parcelbase_SSL" = "SSL"))))
...
allsingle1 %>% 
  filter(!is.na(numberofunits),
         numberofunits != 0,
         !is.na(numberofunits_mar)) %>% 
  mutate(difference = abs(numberofunits_mar - numberofunits)) %>%
  filter(difference != 0) %>% 
  summarize(avg_difference = mean(difference),
            median_difference = median(difference),
            difference_2 = sum(difference <= 2),
            difference_100 = sum(difference >= 100),
...
allsingle1 %>% 
  ...
  filter(difference != 0) %>% 
  ggplot() +
  geom_histogram(mapping = aes(difference),
                 bins = 70)

```
Since there are significant differences- give preference to the variable from the master address repository. Replace `numberofunits` with `numberofunits_mar` unless `numberofunits_mar` is missing or 0.

```{r replace-units}

allsingle2 <- allsingle1 %>% 
  mutate(numberofunits = ifelse((!is.na(numberofunits_mar) | numberofunits_mar != 0),
                                numberofunits_mar,
                                numberofunits))
...
singles_f <- allsingle2 %>% 
  select(county_fips, county_name,
         ...
         assessedlandvalue, assessedimprovementvalue, totalassessedvalue,
         parcelbase_phasebuild, parcelbase_phaseland, parcelbase_mix1txtype, parcelbase_newimpr)

#no longer any missing -commenting out code
#missing_f <- missing %>% 
 # select(county_fips, county_name,
       #  parcelbase_SSL, 
        # propaddress, propcity, propstate, propzip,
         ...
         #parcelbase_address, parcelbase_usecode, parcelgeo_x, parcelgeo_y, 
         #assessedlandvalue, assessedimprovementvalue, totalassessedvalue,
        # parcelbase_phasebuild, parcelbase_phaseland)


multiples_f <- nested_final %>% 
  select(-ends_with("_list"), -lsmode, -lsfirst, 
         -lsmode_num, -fxn, -unique_lots, -lotsize_sf_sum) %>% 
  mutate_at(vars(category, category_detail, building_type, yearbuilt, zoning, parcelbase_usecode, parcelbase_mix1txtype), as.character) %>% 
