...
Note: The following jurisdictions are not inside Fairfax County: 

City of Fairfax

The folowing jurisdictions are inside Fairfax County but not zoned by Fairfax County: 
...
other <- c("CONVERTED RESIDENTIAL OFFICE",
           "MOBILE HOMES IN PARK OR COURT",
           "MOBILE/MANUFACTURED HOME",
           "NEC - OTHER RESIDENTIAL",
           "RURAL RESIDENCE (AGRICULTURAL)"
...
Test different the difference between `area_tax` and `lotsize_sf`. 
...
# identify obs that likely had an error in the unit label in black knight data
area_testing <- jur_1a %>% 
  filter(!is.na(lotsize_sf), !is.na(area_tax)) %>% 
  mutate(conversion_error = ifelse(lotsize_sf / 43560 == area_tax,
                                   1,
                                   0),
         difference = lotsize_sf - area_tax)

# count the number of unit conversion errors
count(area_testing, conversion_error) %>% 
  knitr::kable()

# look more closely at observations that did not have a conversion error, but are still different
area_testing %>% 
  filter(conversion_error == 0) %>% 
  summarize(same_area = sum(lotsize_sf == area_tax),
            different_area = sum(lotsize_sf != area_tax),
            median_difference = median(difference[difference != 0]),
            max_difference = max(difference),
            difference_lt_10 = sum(difference[difference != 0] < 10),
            difference_over_100 = sum(difference > 100)) %>% 
  knitr::kable()

```

Most of the differences between `area_tax` and `lotsize_sf` are small. Replace `lotsize_sf` with `area_tax` if `area_tax` is non-missing. If `lotsize_sf` is still missing, then replace with `parcel_area`. 
...
jur_1 <- jur %>% 
  ...
  # replace lotsize_sf with area_tax when area_tax is not missing and the two values are different
  mutate(lotsize_sf = case_when(
    (is.na(lotsize_sf) & !is.na(area_tax)) | 
      lotsize_sf != area_tax  & !is.na(area_tax) ~ as.double(area_tax),
    TRUE ~ lotsize_sf
  )) %>%
  # reset lotsize_sf to NA if 0
  mutate_at(vars(lotsize_sf), ~ replace(., . == 0, NA)) %>%
  # if lotsize_sf is still missing, use parcel_area
  mutate(lotsize_sf = case_when(
    is.na(lotsize_sf) ~
      as.double(parcel_area),
    TRUE ~ lotsize_sf))  %>%
  # replace variables for number of units
  mutate(num_livunit = case_when(
    num_livunit == 0 | is.na(num_livunit) ~ units_rental,
    TRUE ~ num_livunit
  )) %>% 
  # recreate acres variable
...
nested <- multiples_1 %>% 
  ...
  summarize_at(vars(zoning,lotsize_sf,
                    ...
                    yearbuilt, long, lat,
                    area_tax, zone_tax, possibleSF_tax, stories_tax, units_tax, category_tax, land_per_parcel, dwellings_per_parcel, num_livunit, assessedlandvalue, assessedimprovementvalue, totalassessedvalue, dwellings_per_parcel, units_tax), list) %>% 
  ...
  mutate(nprops = map(zoning_list, length),
         ...
         units_area = map_dbl(units_tax_list, sum, na.rm=T),
         ...
         dwellings_per_parcel = map_dbl(dwellings_per_parcel_list, sum, na.rm=T),
         units_tax = map_dbl(units_tax_list, sum, na.rm=T)
       )
...
##### Zoning
There are 92 instances where there is a tie in the zoning code- examples below.
...
There are no examples of this, but in each case we want to default to the more general case.
...
nested_final <- bind_rows(
  filter(nested_all_1, map(category_detail, length) <= 1),
  novac, bothvac, onevac) %>% 
  select(-test_vacant)

stopifnot(nrow(nested_all_1) == nrow(nested_final))

rm(novac, onevac, bothvac, nested_all, nested_all_1)
```
...
singles_f <- singles_1 %>% 
  select(county_fips, county_name,
         ...
         ends_with("prop"),
         area_tax, zone_tax, possibleSF_tax, stories_tax, category_tax, parcel_id, num_livunit, dwellings_per_parcel, land_per_parcel, assessedlandvalue, assessedimprovementvalue, totalassessedvalue, numberofunits_tax = units_tax)
...
missing_f <- missing %>% 
  select(county_fips, county_name,
         ...
         noofstories, numberofunits, yearbuilt,
         lotsize_acres, lotsize_sf, address_type,
         category, category_detail, residential, building_type, area_tax, zone_tax, possibleSF_tax, stories_tax, category_tax, units_tax, parcel_id, num_livunit, dwellings_per_parcel, land_per_parcel, numberofunits_tax=units_tax, assessedimprovementvalue, totalassessedvalue, assessedlandvalue)
...
multiples_f <- nested_final %>% 
  ...
  rename(lotsize_sf = lotsize_sf_final, numberofunits = nprops, numberofunits_tax = units_tax) %>% 
  mutate(lotsize_acres = lotsize_sf / 43560,
         address_type = "multiple")

jur_4 <- bind_rows(singles_f, multiples_f, missing_f) %>% 
  fill(county_fips) %>% 
  fill(county_name) %>%
  select(-list_no_na)

jur_4[jur_4 == "NULL"] = NA
jur_4[jur_4 == "character(0)"] = NA

```

#####Quick fix to category 
```{r}
jur_4 <- jur_4 %>% mutate(category = case_when(category=="c(\"other\", \"sf\")"~"other", T~category))
```
