...
Make function to sum missing units 
...
Make function to sum missing lotsize 
...
arlington1 <- arlington %>% 
  mutate(lotsize_sf = ifelse(county_name == "Arlington" & is.na(lotsize_sf),
                             parcel_area,
                             lotsize_sf),
...
fairfax2 %>% 
  group_by(category_detail) %>% 
  summarize_at(vars(area_tax, parcel_area),
               ~ sum(is.na(.)) / n()) %>% 
  knitr::kable()
...
Units were already filled in `clean-DC.Rmd`
