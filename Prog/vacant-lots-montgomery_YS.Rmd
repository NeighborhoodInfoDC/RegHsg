...
```{r read}
if (!exists("jur")) {
  jur <-  read_csv (paste0("L:/Libraries/RegHsg/Data/", filepath,
...
zoning_gaithersburg2 <- zoning_gaithersburg1 %>% 
  ...
  mutate(dwelling_type = factor(dwelling_type,
        ...
        mutate(housing_type= case_when(zoning_code %in% SF_g ~ "SF", 
                                       ...
                                       zoning_code %in% COM_g ~ "Commerical",
...
#gaithershburg also has R200(5) RE2C(1) which should be a montegomery codes -- what to do? 
...
filter(housing_type=="SFA") %>% 
 filter(category=="vacant") %>% 
  ...
  select(zoning_code, propaddress, dwelling_type, lotsize_sf, height_ft, lot_coverage_max, units_height, buildable_area, living_area, units_total, units_per_floor, floors, one_unit_flag,  subset, lot_area_min, lotarea_per_unit, smallsize_flag, zone)
...
get_unit_variables_r(dataset =  zoningdata_rockvile1_final2,
                 construction_efficiency = .7,
                 average_unit_size = 940) %>% 
  saveRDS("L:/Libraries/RegHsg/Data/montgomery/Rockville-max-units.Rdata")
...
get_unit_variables_g <- function(dataset, 
  ...
  dataset %>% 
    ...
    mutate(subset=as.numeric(floor(lotsize_sf/lot_area_min))) %>% 
    mutate(units_total= pmin(units_height, units_lotsize, na.rm = TRUE )) %>% 
    mutate( units_total = ifelse(is.na(units_total)==1|units_total > (density *(lotsize_sf/43560)), floor(density *(lotsize_sf/43560)),units_total))
...
testg <- get_unit_variables_g(dataset = zoningdata_gaithersburg1_final,
  ...
  filter(housing_type=="SF") %>% 
   filter(category=="vacant") %>% 
  filter(smallsize_flag==0) %>% 
  select(zoning_code, propaddress, subset, lot_area_min, lot_area_min, lot_coverage_max, lotsize_sf, lotarea_per_unit, height_ft, density,  units_per_floor, living_area, floors, units_height, units_lotsize, units_total, one_unit_flag, two_unit_flag,  smallsize_flag)
...
get_unit_variables_r(dataset =  zoningdata_gaithersburg1_final2,
                 construction_efficiency = .7,
                 average_unit_size = 940) %>% 
  saveRDS("L:/Libraries/RegHsg/Data/montgomery/Gaithersburg-max-units.Rdata")
