...
anti_join(total_rockvile1, zoning_rockville1, by = c("zoning_code")) %>% 
  count(zoning_code) %>% 
  knitr::kable()
# Rockvile flag still has codes from other jurisdiciton ( R20 etc?) Patrick-- check if the flag is complete
...
anti_join(total_gaithersburg, zoning_gaithersburg2, by = c("zoning_code")) %>% 
  count(zoning_code) %>% 
  knitr::kable()
#gaithershburg also has R200 which should be a montegomery code
