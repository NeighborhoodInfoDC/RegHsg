...
get_unit_variables_g <- function(dataset, 
  ...
  dataset %>% 
    ...
    mutate( units_total = ifelse(is.na(units_total)==1, (density *lotsize_sf/43560),units_total)) %>% 
    mutate (units_total = case_when(
...
testg <- get_unit_variables_g(dataset = zoningdata_gaithersburg1_final,
                 construction_efficiency = .7,
                 average_unit_size = 940) %>% 
  filter(zoning_code=="MXD") %>% 
  select(zoning_code, lotsize_sf, lotarea_per_unit, height_ft, density, units_height, units_lotsize, one_unit_flag, units_total, smallsize_flag)
