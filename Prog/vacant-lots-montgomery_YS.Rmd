...
zoningdata_gaithersburg2 <- left_join(vacant_gaithersburg1, zoning_gaithersburg2, by = "zoning_code")
...
zoningdata_rockvile3 <- zoningdata_rockvile2 %>% 
...
zoningdata_gaithersburg3 <- zoningdata_gaithersburg2 %>% 
  filter(!is.na(zoning_code),
         lotsize_sf >= lot_area_min) %>% 
  group_by(propaddress) %>% 
  slice(which.min(dwelling_type)) %>% 
