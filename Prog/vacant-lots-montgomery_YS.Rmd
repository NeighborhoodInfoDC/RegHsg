...
```{r no-non-res}
#YS edits
# YS: need to either merge the jurisdiction zoning code back together now or merge after all cleaned. 
dual <- dual %>% 
...
zoningdata <- left_join(vacantfinal, zoning_rockville1, by = "zoning_code")
...
```{r antijoin}
#YS edits
anti_join(vacantfinal, zoning_rockville1, by = c("zoning_code")) %>% 
  count(zoning) %>% 
  knitr::kable()
