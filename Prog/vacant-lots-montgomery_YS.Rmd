...
#Gaithersburg: There are still montgomery codes under the new Gaithersburg flag: R200. DOn't know what RMH RMX are. 
...
test <-anti_join(vacant_gaithersburg1, zoning_gaithersburg2, by = c("zoning_code")) %>% 
  count(zoning_code) %>% 
  knitr::kable()
...
issuelotgaithersburg<- vacant_gaithersburg2 %>% 
  filter(zoning_code=="RMX"|zoning_code=="RMH") %>% 
...
zoningdata_rockvile1 <- left_join(vacant_rockvile1, zoning_rockville1, by = "zoning_code")

zoningdata_gaithersburg1 <- left_join(vacant_gaithersburg1, zoning_gaithersburg2, by = "zoning_code")


count(zoningdata_rockvile1, zoning_code)

count(zoningdata_gaithersburg1, zoning_code)
```
...
zoningdata_rockvile1_final <- zoningdata_rockvile1 %>% 
...
zoningdata_gaithersburg1_final <- zoningdata_gaithersburg1 %>% 
...
Rockville <- get_unit_summary_r(dataset = zoningdata_rockvile1_final,
                 construction_efficiency = .7,
                 average_unit_size = 940,
                 dwelling_type_breakdown = TRUE) 
...
Gaithersburg <- get_unit_summary_g(dataset = zoningdata_gaithersburg1_final,
                 construction_efficiency = .7,
                 average_unit_size = 940,
                 dwelling_type_breakdown = TRUE) 
