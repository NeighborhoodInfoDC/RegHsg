...
#YS edits
sfcodes <- c("R-400","R-200","R-150","R-90","R-75", "R-60", "R-40")

sfacodes <- c("RMD-10", "RMD-15", "RMD-25")

mfcodes <- c("MXTD", "MXCD", "MXE", "MXB", "MXCT", "MXNC", "MXC", "MXTD")
...
get_unit_summary_r(dataset =  zoningdata_rockvile1_final2,
                 construction_efficiency = .7,
                 average_unit_size = 940,
                 dwelling_type_breakdown = TRUE) %>% 
...
get_unit_variables_g <- function(dataset, 
  ...
  dataset %>% 
    mutate(parameters = paste0("CE: ", construction_efficiency * 100,
           ...
           units_height= ifelse(is.na(units_height)==1, 0, units_height),
           units_lotsize = floor(lotsize_sf / lotarea_per_unit),
           units_lotsize= ifelse(is.na(units_lotsize)==1, 0, units_lotsize)) %>% 
    mutate(subset=floor(lotsize_sf/lot_area_min)) %>% 
    mutate(units_total = case_when(
...
get_unit_summary_g <- function(dataset, 
  ...
  if (dwelling_type_breakdown == TRUE) {
      ...
      bind_rows(y, mutate(x, dwelling_type = "Sum"))  
...
#YS comment: multiple family condominium dwelling ()
...
get_unit_summary_g(dataset =  zoningdata_gaithersburg1_final2,
                 construction_efficiency = .7,
                 average_unit_size = 940,
                 dwelling_type_breakdown = TRUE) %>% 
  write_csv(paste0("L:/Libraries/RegHsg/Prog/Tables/",
