...
maxunits <- bind_rows(fair, mont, rock, gait, arlbr)
...
dcmax <- dc %>% 
  left_join(dcp, by = c("county_fips", "county_name",
                        "parcelbase_SSL" = "assessorsparcelnumberapnpin",
                        "propaddress", "propcity"))


allmax <- maxunits %>% 
  left_join(filter(parcel, county_fips != "11001"),
...
mfsum <- mfmax %>% 
  ...
  summarize(total_mf = n(),
            ...
            under_utilized = sum(under) / n(),
            total_under30p = sum(under30),
            under_30p = sum(under30) / n(),
