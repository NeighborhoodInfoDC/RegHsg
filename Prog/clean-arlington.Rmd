...
weird1 <- nested %>% 
  ...
  select(instance, propaddress, nprops, lotsize_sf_list, lotsize_sf) %>% 
  arrange(instance)
...
nested1 <- nested %>% 
  ...
  filter(lotsize_sf_sum != 0)

always_sum <- nested1 %>% 
  filter(nprops == unique_lots)

always_first <- nested1 %>% 
  filter(unique_lots == 1)

everything_else <- nested1 %>% 
  filter(!propaddress %in% always_sum$propaddress,
         !propaddress %in% always_first$propaddress)
```
