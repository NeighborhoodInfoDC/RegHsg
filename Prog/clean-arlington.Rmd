...
nested <- multiples %>% 
  group_by(propaddress, parcel_address) %>%
  summarize_at(vars(zoning, lotsize_acres, lotsize_sf,
  ...
  mutate(nprops = map(zoning_list, length),
         ...
         yearbuilt = map(yearbuilt_list, max, na.rm = FALSE),
...
```{r row-check}
stopifnot(nrow(nested_all) == nrow(nested1))

rm(nested, nested1, nested_done)
```
#### Ties
...
ties %>%
  ...
  head(5)
```
These will likely have to be figured out manually. Write out the file to determine how to break the ties. For now, we exclude these 62 properties from the final dataset.
...
Run one last check to make sure everything from the initial dataset is accounted for. We have to account for the fact that the addresses were collapsed and the ties were removed.
...
t <- ties %>% 
  summarize(num = sum(nprops)) %>% 
  pull()

other <- finaljur %>% 
  filter(address_type != "multiple")

stopifnot(mult + t +nrow(other) == nrow(jur))
