...
First- summarize and create a list, so we can see all the cases where there is not a unique partial parcel number for each address.
```{r parcel}
parcels <- currentjur_cat %>% 
  group_by(propaddress) %>% 
  summarize(parcel = list(assessorsparcelnumberapnpin)) %>% 
  mutate(mult_parcel = ifelse(map(parcel, length) > 1,
                              1,
                              0)) %>% 
  filter(mult_parcel == 1)
...
First- look at the distribution of lot sizes. We are looking for two things: outliers that indicate that `sum()` was not the right operation- and if there is any obvious cut off point where we can stop using `sum()` and switch to `Mode()`

```{r lot-hist}

ggplot(data = currentjur_cat2, mapping = aes(x = lotsize_sf)) +
  geom_histogram()

```

There are big variations by property type.
```{r lot-facet}
...
Based on the values in the distributions, and trends observed from apartment building lot sizes (which are recorded with one observation per building), we create the following framework as a cut of for the switch from `sum()` to `Mode()`.
