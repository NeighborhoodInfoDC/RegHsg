...
vacant1 <- vacant %>% 
  ...
  mutate(missing_coord = ifelse(is.na(lat) | is.na(long),
                                1,
                                0),
         parcel_x= parcelgeo_x/1000,
         parcel_y= parcelgeo_y/1000,
...
#this should work but doesn't for now. Will come back to this, use blacknight for joining for now
...
We need to clean the spatial joined zoning code, especially the R started ones that have a dash in the middle that are not joined with the zoning spreadsheet. After cleaning, use anti_join again to test if there are any left over residential codes that are not joined.
...
There are 2026 R1s that are not joined which is pretty significant, in our spreadsheet we have R1A and R1B, pull in lotsize and see which ones we should assign the R1 to. R1A has lot size of 7500, R1B has lot size of 5000. Use as threshold.
```{r assign R1}
...
Filter out lots that are too small to build on and lots with missing zoning information, and select most dense zoning code for those that are big enough.Notice after filtering out lotsize_sf>= lot_area_min, observation dropped from 27166 to 6969, after slicing out the least dense zoning code, we only have 1640 observation left. We need to fix this.
...
# 13428 cases where lotsize_sf is less than the minimum lot area or missing. 
...
ggplot(zoningdata1, aes((lotsize_sf-lot_area_min)) )+
  geom_histogram(binwidth = 500)
