...
zoningcode1 <- zoningcode %>% 
  mutate(newdwelling_type = case_when(
    ...
    district_type== "Special Purpose" ~ "Special purpose",
...
zoningcode2 <- zoningcode1 %>% 
  ...
  filter(!(dwelling_type %in% c("All Other", "All")
           & n() > 1)) %>%
  ungroup() 
...
zoningcode_factor <- zoningcode2 %>% 
  mutate( newdwelling_type = factor(newdwelling_type,
                                levels = c("RA", "Mixed", "Other Mixed", "All Other", "Special purpose",
                                           "RF4-C","RF4-B", "RF4-A",
                                           "RF3-C","RF3-B","RF3-A",
                                           "RF2-C","RF2-B","RF2-A",
...
We are switching to blacknight data here:
...
#|:----------------------|-----------:|----------:|------:|-----:|:-------------------|:------------------|:--------------|
#|Vacant unimproved      |        1279|        784|   1471| 11565|11.06%              |6.78%              |12.72%         |
#|Vacant with structures |          12|          4|     27|   242|4.96%               |1.65%              |11.16%         |
...
#this is what you should be getting: [1] 788
```
...
vacant1 %>% 
  ...
  summarize_at(vars(propaddress),
               ~ sum(!is.na(.))) %>% 
  knitr::kable(caption = "Number of addresses that are missing XY coordinates from parcel base")
...
vacant1 %>%
  ...
  summarize_at(vars(propaddress),
               ~ sum(!is.na(.))) %>% 
  knitr::kable(caption = "Number of addresses that are missing XY coordinates but not lat lon from blacknight")

#so there are 1068 observations that we need to use the blacknight location for the join
...
ggplot()+
  geom_sf(data = vacantsfbk, mapping = aes(),
          color = "black", size=0.1)
...
vacantjoin_parcel <- st_join(vacantsfparcel, zon, join = st_intersects)

vacantjoin_bk <- st_join(vacantsfbk, zon, join = st_intersects)


```

After the join, we convert back to a dataframe and recombine the two vacant datasets together, for ease of computations.

```{r back-to-df}

vacant2_parcel <- st_set_geometry(vacantjoin_parcel, NULL)

vacant2_bk <- st_set_geometry(vacantjoin_bk, NULL)

vacant2 <- vacant2_parcel %>% 
     bind_rows(vacant2_bk)
...
```{r real-final}
# report out that the dataset has 0 observations
dual1 <- vacanttotal %>% 
...
## Merge with cleaned vacant lot data
...
vacantcleaned <- vacanttotal %>% 
          mutate(zoning_code=ZONING_LAB, 
                 zoning_bk= zoning) %>% 
          mutate(firsttwo= substr(ZONING_LAB, 1,2),
                 badrescode=ifelse(str_detect(firsttwo, "R-")==1, 1, 0),
                 zoning_code=ifelse(badrescode==1, paste0(substr(ZONING_LAB, 1,1),substr(ZONING_LAB,3,3)),ZONING_LAB))
...
After cleaning the zoning code from shapefile, we are merging the cleaned blacknight vacant data with the zoning code spreadsheet.

```{r zoning-merge}

zoningdata <- left_join(vacantcleaned2, zoningcode_factor, by = "zoning_code") %>% 
           filter(newdwelling_type!="NA")
#filter out observations with zoning codes that are eithing NA or not in the DC spreadsheet
...
Filter out lots that are too small to build on and lots with missing zoning information, and select most dense zoning code for those that are big enough.Notice after filtering out lotsize_sf>= lot_area_min, observation dropped from 25786 to 11318, after slicing out the least dense zoning code, we have 2775 observation left. 
...
# 13580 cases where lotsize_sf is less than the minimum lot area or missing. 
zoningdata1test <- zoningdata %>% 
     filter(lotsize_sf < lot_area_min) %>% 
     mutate(gap= lotsize_sf-lot_area_min) %>% 
    select(lotsize_sf,lot_area_min, gap)

nrow(zoningdata1test)
...
get_unit_variables <- function(dataset, 
  ...
  dataset %>% 
    ...
    mutate(units_total = case_when(
      ...
      units_height > units_FAR ~ units_FAR,  #taking the smaller estimate between the two
      TRUE ~ units_height))
