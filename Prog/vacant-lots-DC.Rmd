...
First, we look at all the options, these categories are not as helpful because attached and detached can mean so many different things in the context ofdifferent zoning types plus we have a lot of NAs for residential code. So need to create a new dwelling type variable.
...
# 13428 cases where lotsize_sf is less than the minimum lot area
...
zoningdata1 %>% 
  get_unit_variables(construction_efficiency = .7,
                     average_unit_size = 900) %>% 
             select(lotsize_sf, units_total, one_unit_flag, two_unit_flag)%>% 
