...
# 
# zoningdata2 %>% 
#   group_by(zoning_code, newdwelling_type, parcelbase_usecode) %>% 
#   count() %>% 
#   knitr::kable()
# 
# 
...
get_unit_variables <- function(dataset, 
  ...
  dataset %>% 
    mutate(parameters = paste0("CE: ", construction_efficiency * 100,
           ...
           FAR_stories = floor_area_ratio_max/lot_coverage_max,
           floors = floor(height_ft /10),
          stories = ifelse(FAR_stories>=floors, floors, FAR_stories),
          stories = as.numeric(case_when(is.na(floors) ~ FAR_stories,
                               is.na(FAR_stories) ~ floors,
                               TRUE ~ stories)),
           stories=ifelse(newdwelling_type=="Mixed"|newdwelling_type=="Special purpose-All"|newdwelling_type=="Special purpose-B", stories-1, stories),
           units_min= units_per_floor * stories,
           units_FAR= units_per_floor * FAR_stories,
    ...
    mutate(units_min=ifelse(is.na(units_min)==1, 0, units_min)) %>% 
    mutate( units_total = case_when(
      one_unit_flag == 1 & subset<1 & units_min>=1 ~ 1,
      two_unit_flag == 1 & subset<1 & units_min>=2 ~ 2,
      three_unit_flag == 1 & subset<1 & units_min>=3 ~ 3,
      four_unit_flag == 1 & subset<1 & units_min>=4 ~ 4,
      ...
      units_height > units_FAR & !is.na(units_FAR) ~ units_FAR,
      TRUE ~ units_height))
}
...
testvacant <- get_unit_variables(dataset = vacantlotunits,
                 construction_efficiency = .7,
                 average_unit_size = 940) %>% 
  filter(housing_type=="SF-attached") %>% 
  select(parcelbase_SSL, lotsize_sf, buildable_area, living_area, height_ft, floors, FAR_stories,             units_per_floor, units_min, units_FAR, units_height, units_total, propaddress, zoning_code ) 
 

get_unit_variables(dataset = vacantlotunits,
                 construction_efficiency = .7,
                 average_unit_size = 940) %>% 
  select(parameters, parcelbase_SSL, lotsize_sf, buildable_area, living_area, height_ft, floors, FAR_stories,             units_per_floor, units_min, units_FAR, units_height, propaddress, zoning_code ) %>% 
  write_csv(paste0("L:/Libraries/RegHsg/Prog/Tables/",
                   filepath,
                   "-vacant-lots-analysis-basedata.csv"))

jurzoningdata3 %>% 
  group_by(category, category_detail) %>% 
  count() %>% 
  knitr::kable()
