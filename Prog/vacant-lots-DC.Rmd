...
#apply the original dwelling type to zoning codes, despite missing dwelling type info for some observations
zoningcode_factor2 <- zoningcode2 %>% 
  mutate(dwelling_type = factor(dwelling_type,
                                levels = c("IZ attached",
                                           "attached",
                                           "IZ semi-detached",
                                           "semi-detached",
                                           "Duplex",
...
zoningdata <- left_join(vacantfinal, zoningcode_factor2, by = "zoning_code")


unique(zoningcode_factor$newdwelling_type)

#after joining, we see that there's no vacant lot that's zoned RF4 RF5
zoningdata %>% 
...
anti_join(vacantfinal,zoningcode_factor2, by = c("zoning_code")) %>% 
  count(ZONING_LAB) %>% 
  knitr::kable()
...
After cleaning, use anti_join again to test if there are any left over residential codes that are not joined. R1 and RC-1, RC-3 not joined. Check the spreasheet and they are not in. 
```{r antijoinagain}

anti_join(vacantcleaned, zoningcode_factor2, by = c("zoning_code")) %>% 
  count(zoning_code) %>% 
  knitr::kable()

```

There are 2228 R1s that are not joined which is a lot, in our spreadsheet we have R1A and R1B, pull in lotsize and see which ones we should assign the R1 to. R1A has lot size of 7500, R1B has lot size of 5000. Use as threshold.
```{r assign R1}
...
anti_join(vacantcleaned2, zoningcode_factor2, by = c("zoning_code")) %>% 
  filter(zoning_code=="R1") %>% 
  count(zoning_code) %>% 
  knitr::kable()

zoningdatanew2 <- left_join(vacantcleaned2, zoningcode_factor2, by = "zoning_code")
...
Filter out lots that are too small to build on and lots with missing zoning information, and select most dense zoning code for those that are big enough.Notice after filtering out lotsize_sf>= lot_area_min, observation dropped from 27576 to 12515, after slicing out the least dense zoning code, we only have 1508 observation left. Part of the reason is we have duplicated propaddress???
          ...
          #lotarea_per_unit is missing for all in DC, so use units_height and units_FAR for units estimates
           ...
           mutate_at(vars(units_height, units_FAR),
              funs(case_when(
                ...
                four_unit_flag == 1 ~ 4,
                TRUE ~ .))) %>% 
    mutate(subset=as.integer(lotsize_sf/lot_area_min), na.rm = TRUE) %>% 
    mutate(units_total = case_when(
      one_unit_flag == 1 & subset<1 ~ 1,
      two_unit_flag == 1 & subset<1~ 2,
      three_unit_flag == 1 & subset<1~ 3,
      four_unit_flag == 1 & subset<1~ 4,
      one_unit_flag == 1 & subset>=1 ~ 1*subset,
      two_unit_flag == 1 & subset>=1~ 2*subset,
      three_unit_flag == 1 & subset>=1~ 3*subset,
      four_unit_flag == 1 & subset>=1~ 4*subset,
...
get_unit_summary <- function(dataset, 
  ...
  y <- dataset %>% 
    get_unit_variables(construction_efficiency,
                       average_unit_size) %>%
    group_by(parameters, newdwelling_type) %>% 
    summarize(lots = n(),
              units_height = sum(units_height, na.rm = TRUE),
              units_FAR = sum(units_FAR, na.rm = TRUE),
              units_total = sum(units_total, na.rm = TRUE),
              units_per_lot = floor(sum(units_total / lots))) %>% 
    mutate(newdwelling_type = as.character(newdwelling_type))
...
zoningdata1 %>% 
  ...
  select(lotsize_sf, newdwelling_type, dwelling_type, units_total)%>% 
  ggplot() +
  geom_point(mapping = aes(lotsize_sf, units_total,
