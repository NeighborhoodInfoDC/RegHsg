...
See how many lots have missing key variables by address. 
...
#so there are 308 observations that we need to use the blacknight location for the join
...
After the join, we convert back to a dataframe and recombine the two vacant datasets together, for ease of calculation.
...
vacanttotaltest <- vacant2 %>% 
  mutate(zoning_code=ZONING_LAB)

anti_join(vacanttotaltest, zoningcode_factor, by = c("zoning_code")) %>% 
  count(zoning_code) %>% 
  knitr::kable()

vacantcleaned <- vacant2 %>% 
          mutate(zoning_code=ZONING_LAB, 
                 zoning_bk= zoning) %>% 
...
Filter out lots that are too small to build on and lots with missing zoning information, and select most dense zoning code for those that are big enough.Notice after filtering out lotsize_sf>= lot_area_min, observation dropped from 15942 to 8140, after slicing out the least dense zoning code, we have 1356 observation left. 
...
# 7790 cases where lotsize_sf is less than the minimum lot area or missing. 
...
zoningdata3 <-zoningdata2 %>% 
  mutate(dwelling_type2= case_when(newdwelling_type=="RA" ~ "Multifamily", #residential apartments
                                   newdwelling_type=="Mixed"|newdwelling_type=="Special purpose-All"| newdwelling_type=="Special purpose-B"~ "Mixed use and Special Purpose Multifamily",
                                   newdwelling_type=="RF2-C" ~ "Semi-detached", #RF-1/2/3 row houses
                                   newdwelling_type=="One Family-F" ~ "Semi-detached", #R3 row dwelling
                                   newdwelling_type=="One Family-E" ~ "Semi-detached", #R2 semidetached
                                   newdwelling_type=="One Family-A" ~ "Single Family" #residential house detached
                                   ))
...
get_unit_variables <- function(dataset, 
  ...
  dataset %>% 
    mutate(parameters = paste0("CE: ", construction_efficiency * 100,
           ...
           floors = as.integer(height_ft /10),
           floors=ifelse(newdwelling_type=="Mixed"|newdwelling_type=="Special purpose-All"|newdwelling_type=="Special purpose-B", floors-1, floors),
           units_height = units_per_floor * floors,
           GFA = floor_area_ratio_max * buildable_area,
           living_area_far=GFA*construction_efficiency,
           units_per_floor_far=living_area_far/average_unit_size,
           units_FAR= units_per_floor_far * floors) %>% 
    ...
    mutate(units_height=ifelse(is.na(units_height)==1, 0, units_height),
           units_FAR=ifelse(is.na(units_FAR)==1, 0, units_FAR)) %>% 
...
get_unit_summary <- function(dataset, 
  ...
  y <- dataset %>% 
    get_unit_variables(construction_efficiency,
                       average_unit_size) %>%
    group_by(parameters, dwelling_type2) %>% 
    summarize(lots = n(),
              units_height = sum(units_height, na.rm = TRUE),
              units_FAR = sum(units_FAR, na.rm = TRUE),
              units_total = sum(units_total, na.rm = TRUE),
              units_per_lot = floor(sum(units_total / lots))) %>% 
    mutate(dwelling_type2 = as.character(dwelling_type2))
  
  
  if (dwelling_type_breakdown == TRUE) {
   
      bind_rows(y, mutate(x, dwelling_type2 = "All"))  
...
get_unit_summary(dataset = zoningdata3,
...
calculateunit_data <- get_unit_variables(dataset = zoningdata3,
                 construction_efficiency = .7,
                 average_unit_size = 900) %>% 
               select(parameters,lotsize_sf, buildable_area, living_area, height_ft, floors, units_height, units_FAR, units_per_floor_far, units_total, propaddress, zoning_code )


get_unit_variables(dataset = zoningdata3,
                 construction_efficiency = .7,
...
zoningdata3 %>% 
  ...
  select(lotsize_sf, newdwelling_type, dwelling_type2, units_total)%>% 
  ggplot() +
  geom_point(mapping = aes(lotsize_sf, units_total,
                           color = dwelling_type2),
             alpha = .8) +
  scale_x_continuous(labels = scales::comma,
