...
We need to clean the spatial joined zoning code that are not joined in the table and join again. 

```{r remove dash}

vacantcleaned <- vacantfinal %>% 
          separate( zoning_code, sep = "-", remove = TRUE, into = c("a1", "a2", "a3")) %>% 
          mutate(zoning_code_new= paste0(a1, a2, a3)) %>% 
          filter(ZONING_LAB=="R-21")


zoningdataclean <- left_join(vacantcleaned, zoningcode1, by = "zoning_code")
