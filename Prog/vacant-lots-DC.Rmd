...
zoningcode_factor <- zoningcode2 %>% 
  mutate( newdwelling_type = factor(newdwelling_type,
                                levels = c("RA", "Mixed", "Other Mixed", "All Other", "Special purpose-All","Special purpose-B","Special purpose-A",
                                           "RF4-C","RF4-B", "RF4-A",
                                           "RF3-C","RF3-B","RF3-A",
                                           "RF2-C","RF2-B","RF2-A",
...
Filter out lots that are too small to build on and lots with missing zoning information, and select most dense zoning code for those that are big enough.Notice after filtering out lotsize_sf>= lot_area_min, observation dropped from 27472 to 11415, after slicing out the least dense zoning code, we have 2807 observation left. 
...
# 13987 cases where lotsize_sf is less than the minimum lot area or missing. 
...
testzoning <- zoningdata1 %>% 
  filter(newdwelling_type=="Special purpose-B") %>% 
  select(lotsize_sf, lot_area_min, lot_coverage_max, floor_area_ratio_max, height_ft, zoning_code,  propaddress)


```

filter out special zoning codes that are not for housing
...
get_unit_variables <- function(dataset, 
  ...
  dataset %>% 
    mutate(parameters = paste0("CE: ", construction_efficiency * 100,
           ...
           units_per_floor_far=living_area/average_unit_size,
