...
get_unit_variables <- function(dataset, 
  ...
  dataset %>% 
    ...
    mutate(units_total = case_when(
      ...
      units_height > units_FAR & !is.na(units_FAR) ~ units_FAR,  #taking the smaller estimate between the two
      TRUE ~ units_height))
...
calculateunit_data <- get_unit_variables(dataset = zoningdata3,
                 construction_efficiency = .7,
                 average_unit_size = 900) %>% 
               select(parameters,parcelbase_SSL, lotsize_sf, buildable_area, living_area, height_ft, floors, units_height, units_FAR, units_per_floor_far, units_total, propaddress, zoning_code  )
