...
Read in zoning layer, set CRS. Also read in parks shapefile for identifying vacant lots that are parks. 
```{r read-zoning}
...
st_geometry(DCparks)
DCparks <- st_transform(DCparks, crs = 4326)
...
st_geometry(nationalpark)
nationalpark <- st_transform(nationalpark, crs = 4326)
...
## Filtering vacant lots that are educational, Cemeteries and parks

Some vacant lots in DC are cemetery, lots on campuses or parks that can't be built upon. We are identifying those cases by two sources: using the tax type information to filter our cemeteries and educational. For parks, we will be joining the parcels with parks shapefile and filter out or at least flag the one that are within a park. 

### Educational and cemetery
```{r filter-tax type}

jurzoningdata4 <- jurzoningdata3 %>% 
  filter(parcelbase_mix1txtype!= "E2"& parcelbase_mix1txtype!= "E7")
...
#convert dataset back to sf
jurzoningdata4_parcel <- jurzoningdata4 %>% 
    filter(missing_parcelgeoXY == 0) %>% 
  st_as_sf(coords = c("parcelgeo_x", "parcelgeo_y")) %>% 
  st_set_crs(2804) %>% 
  st_transform(4326)

jurjoin3_parks <- st_join(jurzoningdata4_parcel, DCparks, join = st_intersects)
...
jur3_parks <- st_set_geometry(jurjoin3_parks, NULL) %>% 
           filter(is.na(PROPID)==0) 

#create a flag=1 
jur3_parks$parkflag <- 1
...
### National Parks
```{r join with national parks}


#Do the same thing for national parks

jurjoin3_nationalparks <- st_join(jurzoningdata4_parcel, nationalpark, join = st_intersects)

jur3_nationalparks <- st_set_geometry(jurjoin3_nationalparks, NULL) %>% 
           filter(is.na(ALPHA_CODE)==0) 

#create a flag=1 
jur3_nationalparks$nationalparkflag <- 1

nationalparkflag <- jur3_nationalparks %>% 
...
jurzoningdata5 <- left_join (jurzoningdata4, parkflag, by = "parcelbase_SSL")

jurzoningdata6 <- left_join (jurzoningdata5, nationalparkflag, by = "parcelbase_SSL")

#14 vacant lots are now flagged as DC parks
testpark <- jurzoningdata6 %>% 
  filter(parkflag==1) %>% 
  filter(category=="vacant") %>% 
  select(propaddress, zoning_code,nationalparkflag, parkflag)

#44 lots are flagged as national park. one lot is both national and DC park. 
testnationalpark <- jurzoningdata6 %>% 
  filter(nationalparkflag==1) %>% 
  filter(category=="vacant") %>% 
  select(propaddress, zoning_code, nationalparkflag, parkflag)

jurzoningdata7 <- jurzoningdata6 %>% 
  filter(is.na(parkflag)) %>% 
  filter(is.na(nationalparkflag))
...
get_unit_variables(dataset = vacantlotunits,
                 construction_efficiency = .7,
                 average_unit_size = 940) %>% 
  select(parameters, parcelbase_SSL, parcelbase_usecode,lotsize_sf, lotsize_sf, buildable_area, living_area, height_ft, floors, FAR_stories,units_per_floor, units_min, units_FAR, units_height, propaddress, zoning_code, one_unit_flag, two_unit_flag, subset, parcelbase_mix1txtype) %>% 
...
get_unit_variables(dataset = jurzoningdata7,
                 construction_efficiency = .7,
                 average_unit_size = 940) %>% 
...
DCmaxunit <- get_unit_variables(dataset = jurzoningdata7,
                 construction_efficiency = .7,
                 average_unit_size = 940) %>% 
  mutate(units_bk= as.numeric(case_when(category== "vacant" ~ 0,
                                        category=="sf"~ 1,
