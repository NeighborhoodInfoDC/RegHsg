...
write_csv(zoning_rockville, paste0("L:/Libraries/RegHsg/Doc/zoning-codes/rockville-zoning-code-clean.csv"))

zoning_gaithersburg <- zoningcode %>% 
  filter(juristdiction == "Gaithersburg")

write_csv(zoning_gaithersburg, paste0("L:/Libraries/RegHsg/Doc/zoning-codes/gaithersburg-zoning-code-clean.csv"))
...
get_unit_variables <- function(dataset, 
  ...
  dataset %>% 
    mutate(parameters = paste0("CE: ", construction_efficiency * 100,
           ...
           living_area_height = ifelse(is.na(buildable_area_height),
                                       lotsize_sf*construction_efficiency,
                                       buildable_area_height*construction_efficiency),
           ...
           stories_far = ifelse(percent_per_floor_far!=0&!is.na(percent_per_floor_far),
                                ...
                                0),
           #stories_FAR = ifelse(zoning_code %in% c("CR", "CRT", "CRN"),
                          #stories_far-1,#subtract one floor if it's mixed use
                          #stories_far),
           subset = ifelse(is.na(lot_area_min), 0, floor(lotsize_sf/lot_area_min)),
           units_height = units_per_floor_height * floors_height,
    ...
    mutate(units_total = case_when(
       one_unit_flag == 1 ~ 1,
       two_unit_flag == 1 ~ 2,
       one_unit_flag == 1 & subset>=1 & units_height/subset>=1 ~ 1*subset,
       two_unit_flag == 1 & subset>=1 & units_height/subset>=2 ~ 2*subset,
