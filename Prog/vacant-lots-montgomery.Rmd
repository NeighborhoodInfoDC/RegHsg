...
count(jur, rockflag != 1, gaitflag != 1)
```
...
Load R dataset from Post clean
```{r read}
...
vacant_mont <- jur %>% 
  filter(category == "vacant", rockflag != 1, gaitflag!=1)

count(vacant_mont, rockflag, gaitflag)

count(jur, category == "vacant", rockflag !=1, gaitflag !=1)
...
count(vacant_mont2, category == "vacant", rockflag !=1, gaitflag !=1)
...
vacantfinal <- vacant_mont2 %>% 
  mutate(zoning_code = case_when(
    zoning %in% c("CR1.0","CR0.5", "CR2.0", "CR5.0") ~ "CR",
    zoning %in% c("CRN0.5","CRN1.5", "CR2.0", "CR5.0") ~ "CRN",
    zoning %in% c("CRT0.5","CRT075", "CRT1.5", "CRT1.0", "CRT125") ~ "CRT",
...
```{r count total lots before figuring out which ones we can build on}
zoningdata %>% 
...
count(zoningdata, category_detail)
...
count(zoningdata1, dwelling_type)
...
zoningdata2 <- zoningdata1 %>% 
  ...
  mutate(lot_coverage_max = if_else(is.na(lot_coverage_max) & dwelling_type == "townhouse", .4, lot_coverage_max)) %>% #here I assume a max townhouse density of 40%
...
count(zoningdata1, lot_coverage_max)
count(zoningdata2, lot_coverage_max)

count(zoningdata1, density)
count(zoningdata2, density)
...
get_unit_variables <- function(dataset, 
  ...
  dataset %>% 
    mutate(parameters = paste0("CE: ", construction_efficiency * 100,
           ...
           units_per_floor_FAR = floor(living_area/average_unit_size),
           units_massing = massing,
    ...
    mutate(units_total = case_when(
      one_unit_flag == 1 ~ 1,
      two_unit_flag == 1 ~ 2,
      TRUE ~ pmin(units_lotsize, units_height, units_density, units_FAR, units_massing, na.rm = TRUE)))
...
get_unit_summary <- function(dataset, 
  ...
  x <- dataset %>% 
    ...
    summarize(lots = n(),
              ...
              #units_per_floor_FAR = sum(units_per_floor_FAR, na.rm = TRUE),
...
results <- get_unit_variables(dataset = zoningdata2,
  ...
  select(dwelling_type, zoning_code, units_height, units_lotsize, units_density, units_massing, units_per_floor_FAR, units_FAR, living_area, units_total, lotsize_sf, lot_coverage_max, units_per_floor_FAR, floors) %>% 
  filter(units_total > 0)

results %>% 
  filter(dwelling_type == "townhouse", units_total > 12) %>% 
...
th <- get_unit_variables(dataset = zoningdata2,
                         construction_efficiency = .7,
                         average_unit_size = 940) %>% 
...
ggplot() +
  geom_histogram(th, mapping = aes(units_height),
                 bins = 60)
