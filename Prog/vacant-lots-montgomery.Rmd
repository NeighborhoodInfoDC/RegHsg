...
write_csv(zoning_rockville, paste0("L:/Libraries/RegHsg/Doc/zoning-codes",
                   "rockville-zoning-code-clean.csv"))

zoning_gaithersburg <- zoningcode %>% 
  filter(juristdiction == "Gaithersburg")

write_csv(zoning_gaithersburg, paste0("L:/Libraries/RegHsg/Doc/zoning-codes",
...
montfinal <- mont %>% 
  mutate(zoning_code = case_when(
    ...
    zoning %in% c("RT","RT6", "RT6.0") ~ "RT-6.0",#if there is no number after RT we go with the most restrictive code
...
4. fill in per unit lot size when there is not general minimum lot size restriction
...
get_unit_variables <- function(dataset, 
  ...
  dataset %>% 
    mutate(parameters = paste0("CE: ", construction_efficiency * 100,
           ...
           FAR_stories =ifelse(zoning_code %in% c("CR", "CRT", "CRN"),
                          FAR_stories-1,#subtract one floor if it's mixed use
                          FAR_stories),
           ...
           units_per_floor = floor(living_area/average_unit_size),
           units_FAR = ifelse(is.na(far_max), NA, units_per_floor * FAR_stories)) %>% 
...
testing123 <- resultsall %>% 
  ...
  select(dwelling_type, propaddress, zoning_code, lotsize_sf, buildable_area, living_area, units_per_floor, floors, stories, FAR_stories, far_max, lot_coverage_max,units_total, units_lotsize, units_min, units_height, units_density, units_FAR)
