...
```{r setup}
library(tidyverse)
#library(urbnthemes) #urban themes was causeing bugs, I took it out.
library(sf)
library(ggplot2)
library(naniar)
...
count(jur, rockflag != 1, gaitflag != 1, notinmont == 1)

```

This uses read_montgomery
...
## Create zoning for each jur so we can split the analysis
...
montfinal <- mont %>% 
  mutate(zoning_code = case_when(
    zoning %in% c("CR1.0","CR0.5", "CR2.0", "CR5.0", "cR3", "CR5", "CR3.0", "CR4.0") ~ "CR",
    ...
    zoning %in% c("CRT0.5","CRT075", "CRT1.5", "CRT1.0", "CRT125", "CRT1", "CRT175", "CRT2.25", "CRT2.5", "CRT 1") ~ "CRT",
...
1. filter out non residential properties
2. fill in lot area minimums from site minimums
3. fill in lot coverage maximums from site coverage maximums
4. fill in per unit....*patrick do this 3/20*
5. We also need to fill in missing lot coverage

6. and make two assumptions for townhouses that do not have lot coverage restrictions in the montgomery zoning code.
  1. The highest lot coverage for townhouses elsewhere in the codes is 40, so I assume a lot max of 40.
  2. The highest density allowed for townhouses elsewhere in the codes is 15.02, so I assume this density.
  
7. finally, we need to fill in the lotsize_acre variable for those lot sizes that we filled in from the parcel file.
...
zoningdata1 <- zoningdata %>% 
  ...
  mutate(lot_coverage_max = if_else(is.na(lot_coverage_max) & dwelling_type == "townhouse" & zoning_code %in% c("CRN", "CRT", "CR"), .4, lot_coverage_max )) %>% 
  #I fill in lot coverage with site coverage and assume a maximum townhouse density of 40%
  mutate(density = if_else(is.na(density) & dwelling_type == "townhouse" & zoning_code %in% c("CRN", "CRT", "CR"), 15.02, density)) %>% # here I assume a maximum townhouse density of 15.02
  mutate(lotsize_acres = lotsize_sf/43560) #i've filled in and modified a bunch of lotsize_sf, I make sure acres reflects that.

count(zoningdata1, is.na(lotsize_acres), is.na(lotsize_sf))#check that we got all the acres.
...
count(zoningdata1, is.na(zoning_code))#see how many observations should drop between steps
...
#'
#' @return returns a dataframe with new variables: `parameters`, `buildable_area`, `living_area`, `units_per_floor`, `floors`,`FAR_stories`,`stories`, `units_FAR`,  `units_height`, `units_lotsize`, and `units_total`
#'
#' @examples
get_unit_variables <- function(dataset, 
  ...
  dataset %>% 
    mutate(parameters = paste0("CE: ", construction_efficiency * 100,
           ...
           living_area = ifelse(is.na(buildable_area),
                                lotsize_sf * construction_efficiency,
                                buildable_area * construction_efficiency),
           units_per_floor = floor(living_area / average_unit_size),
           floors = floor(height_ft / 10),
           FAR_stories = ifelse(is.na(lot_coverage_max), floor(far_max), floor(far_max/lot_coverage_max)),
           stories = ifelse(FAR_stories >= floors, floors, FAR_stories) %>% as.double(),
           stories = case_when(is.na(floors)~FAR_stories,
    ...
    mutate(units_min=ifelse(is.na(units_min), 0, units_min)) %>% 
    ...
    mutate(units_total = case_when(
      ...
      TRUE ~ pmin(units_lotsize, units_min, units_height, units_density, units_FAR, na.rm = TRUE)))
...
I created a dataset with results for troubleshooting purposes. Then I checked to make sure there aren't more townhouses than are allowed to be built on any one property.
```{r sumassign}
...
resultsvacant %>% 
  filter(dwelling_type == "apartment", units_total == 0 ) %>% 
  count(zoning_code, units_total)
  
#i've added an additional filter to my function - there are several units where the lotsize passes the first filter, but the buildable area is not large enough to build on - so it creates 0 units. It applies to these units.
...
Here I did trouble shooting for the units that produced no units. ignore
```{r troubleshoot}
th <- get_unit_variables(dataset = vacant_mont2,
                         construction_efficiency = .7,
                         average_unit_size = 940) %>% 
  filter(dwelling_type == "townhouse") %>% 
  filter(units_total == 0, buildable_area >940) %>% 
  select(parameters, propaddress, buildable_area, living_area, zoning_code, lotsize_sf , units_per_floor, floors,FAR_stories,stories, units_FAR, units_height, units_density, units_lotsize, units_min, units_total)

th %>% 
  count(units_total)


thl <- get_unit_variables(dataset = vacant_mont2,
                          construction_efficiency = .7,
                          average_unit_size = 940) %>% 
  filter(units_total > 0, dwelling_type == "apartment", lotsize_sf > 10000, is.na(buildable_area)) %>% 
  select(parameters, propaddress, buildable_area, living_area, zoning_code, lotsize_sf , lotarea_per_unit, units_per_floor, floors,FAR_stories,stories, units_FAR, units_height, units_density, units_lotsize, units_min, units_total)
