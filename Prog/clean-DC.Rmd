...
jur <- jur %>% 
  mutate(parcel_address = substr(assessorsparcelnumberapnpin, 1, 6),
         parcelbase_address= substr(parcelbase_parcelID, 1, 6)) 

bad_parcels <- jur %>% 
  group_by(propaddress) %>% 
  summarize(parcel = list(parcel_address),
            parcelbase= list(parcelbase_address)) %>% 
...
#use parcelbase address to fill in address where possible
...
There are a total of 2469 cases that are not taken care of by the above function. Use basic statisitcs for range of nprops, mode and other variables of interest. After examining the extreme cases, I followed most of Sarah's logic, but had some with the nprops one. 
