...
parceladdress <- jur2 %>% 
  mutate(firstcharacter=str_sub(parcelbase_address,1,1),
    ...
    mutate(propaddress2= str_remove(parcelbase_address, "^0+"),
         ...
         streettest= str_trim(propaddress7, side=c("both")),
         streetnew1= str_extract(streettest, "[[:digit:]]+"),
         streetnew= str_replace(propaddress7, "\\w*[0-9]+\\w*\\s*", "" ),
         wrongaddress2= ifelse(str_detect(streetnew, "[:digit:]")==1,0,1)) 

  
test4 <- parceladdress %>% 
 filter(parcelbase_parcelID=="4445    2018") %>% 
  mutate(idlength= str_length(parcelbase_parcelID))

test5 <- parceladdress %>% 
  select(parcelbase_parcelID, parcelbase_address) %>% 
  separate(parcelbase_address, sep = " ", remove = FALSE,
           into = c("a1", "a2", "a3", "a4", "a5", "a6")) 


test5 %>% pull(parcelbase_address)

alladdress <- parceladdress %>% 
  group_by (propaddress7) 

    
cleanparceladdress <- parceladdress %>% 
...
#aftercleaning the missing category dropped from 9562 to 8715, dropped 847 which matches the total observation of cleanparceladdress dataset
count(jur4, address_type)
