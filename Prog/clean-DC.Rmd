...
if (!file.exists(paste0("../Data/", filepath, "-county-land-use-crosstab.csv"))) {
  write_csv(crosstab_category,
            paste0("../Data/", filepath, "-county-land-use-crosstab.csv"))
}
...
I used this code to check some of the confusing ones in the parcel base data to assign the right category
```{r}
jur %>% 
...
``` {r lot-ex, echo=FALSE}
weird <- tibble(propaddress = c( "1 SCOTT CIR NW", "10 G ST NE", "100 FLORIDA AVE NE", "1000 CONSTITUTION AVE NW",  "1007 OTIS PL NW", "1003 50TH ST NE"),
                instance = 1:6)
...
- IN the case of when the lot size is repeated (Instance 2)- the number of unique lot sizes is 1, so mode is the right function.
- In the case of when there are the same number of unique values as there are properties- sum is the correct function. 
- In the fifth and sixth instance, two smaller number add up to the third. I looked up several online and it seems like best to keep the total, summing all will double count the lot size
...
There are a total of 3225 cases that are not taken care of by the above function. Use basic statisitcs for range of nprops, mode and other variables of interest. After examining the extreme cases, I followed most of Sarah's logic, but had some with the nprops one. 
...
##### Zoning
There are 154 instances where there is a tie in the zoning code- examples below.
