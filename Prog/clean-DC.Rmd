...
This code only applies to blackknight data because parcelbase dataset don't have a prophouse variable.So if we are using address from parcel base dataset the step below don't really apply but keeping it for now. 
...
jur2 %>% 
  ...
  select(oldadd, propaddress, category_detail,parcelbase_address) %>% 
  head(20)
...
# this count shows by using parcelbase_address as is for collasing, we are having much more single addresses that should not be collapsed.
count(jur2, address_type)

#create jur3 from blackknight address
...
nested <- multiples %>% 
  group_by(propaddress, parcelbase_parceladdress) %>%
  summarize_at(vars(zoning,lotsize_sf,
                    buildingarea, parcelbase_usecode,
                    residential, category, category_detail, building_type,
                    yearbuilt, parcelgeo_x, parcelgeo_y), list) %>% 
  
  rename_at(vars(-propaddress, parcelbase_parceladdress), ~ paste0(., "_list")) %>% 
  mutate(nprops = map(zoning_list, length),
         ...
         usecode = map(parcelbase_usecode_list, Mode),
         residential = map(residential_list, max, na.rm = TRUE),
         ...
         building_type = ifelse(category == "mf",
                                ...
                                NA),
         Xcoord = map(parcelgeo_x_list, median),
         Ycoord = map(parcelgeo_y_list, median),
...
weird <- tibble(propaddress = c( "1 SCOTT CIR NW", "100 FLORIDA AVE NE","1007 OTIS PL NW", "816 8TH ST NE"),
                instance = 1:4)
...
- IN the case of when the lot size is repeated (Instance 3)- the number of unique lot sizes is 1, so mode is the right function.
- In the case of when there are the same number of unique values as there are properties- sum is the correct function. 
- In the fourth instance, two smaller number add up to the third. I looked up several online and it seems like best to keep the total, summing all will double count the lot size
...
There are a total of 2392 cases that are not taken care of by the above function. Use basic statisitcs for range of nprops, mode and other variables of interest. After examining the extreme cases, I followed most of Sarah's logic, but had some with the nprops one. 

1. The lotsize seems reasonable, but the nprop is double the actual value. Not sure how to deal with this
2. In addition, I found a case (also presented in the table above) where one lotzie equals the sum of all other lotsize in the same address. I'm not sure how to programmatically detect all cases like this. 
...
multiples_f <- nested_final %>% 
  ...
  mutate_at(vars(category, category_detail, building_type, yearbuilt, zoning, parcelbase_usecode),
            as.character) %>% 
  mutate_at(vars(buildingarea, residential), as.double) %>% 
