...
resconvert <- c("RES - CONVERSIONS - MORE THAN 5 UNITS",
"RES CONVERSIONS - 5 UNITS",
"RES CONVERSIONS - LESS THAN 5 UNITS")

coop <- c("COOPERATIVE - HORIZONTAL - MIXED USE",
"COOPERATIVE - VERTICAL - MIXED USE",
"RES - COOPERATIVE - VERTICAL",
"RES COOPERATIVE - HORIZONTAL")

apartment <- c("RES FLATS - LESS THAN 5 UNITS",
"WALK UP APT")

 resother<- c("FALSE VACANT ABUTTING LOT (RES)",
"RES MIXED USE",
"RESIDENTIAL - MULTIFAMILY MISC",
"RESIDENTIAL TRANSIENT MISC",
"SINGLE FAMILY - RES - NON CONFORMING")
...
vacantother <- c("VACANT - UNIMPROVED PARKING",
"VACANT W/PERMIT",
"VACANT W/ZONING LIMITS")
...
jur <- jur %>% 
  mutate(category_detail = case_when(
           ...
           countylandusedescription %in% resconvert ~ "residential conversion",
           countylandusedescription %in% coop ~ "cooperatives",
           countylandusedescription %in% apartment ~ "rental apartment",
           countylandusedescription %in% resother ~ "misc residential",
           countylandusedescription %in% commercial ~ "commerical",
           countylandusedescription %in% office ~ "office",
           countylandusedescription %in% truevacant ~ "true vacant",
           countylandusedescription %in% vacantcom ~ "vacant commercial",
           countylandusedescription %in% vacantother ~ "vacant other"))

rm(sfd, sfa, condo,resconvert,coop,apartment,resother,commercial,office,truevacant,vacantcom,vacantother)
...
other <- c("RESIDENTIAL - MULTIFAMILY MISC",
"RESIDENTIAL TRANSIENT MISC")
               
jur <- jur %>% 
  mutate(building_type = case_when( countylandusedescription %in% garage ~ "garage",
...
rm(garage,vertical, horizontal, other )           
```
