...
nested_tax %>% count(fxn_tax)

nested_done <- nested_tax %>% 
...
```{r row-check2}
stopifnot(nrow(nested_all) == nrow(nested_tax))

rm(nested_tax, nested_done, nested_other)
```
