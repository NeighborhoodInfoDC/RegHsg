...
nested <- multiples_1 %>% 
  ...
  summarize_at(vars(zoning,lotsize_sf,
                    ...
                    yearbuilt, long, lat,
                    area_tax, zone_tax, possibleSF_tax, stories_tax, units_tax, category_tax, land_per_parcel, dwellings_per_parcel, num_livunit, assessedlandvalue, assessedimprovementvalue, totalassessedvalue), list) %>% 
...
As the lot size variable from the BlackKnight data is missing `r sum(is.na(jur$lotsize_sf))` observations, we would like to do the same methodology with some joined data from Fairfax county to compare whether their size variable contains more information. Fairfax's lotsize variable has `r sum(is.na(jur$area_tax))` missing observations. 
...
singles_f <- singles_1 %>% 
  select(county_fips, county_name,
         ...
         ends_with("prop"),
         area_tax, zone_tax, possibleSF_tax, stories_tax, category_tax, parcel_id, num_livunit, dwellings_per_parcel, land_per_parcel, assessedlandvalue, assessedimprovementvalue, totalassessedvalue)
...
missing_f <- missing %>% 
  select(county_fips, county_name,
         ...
         noofstories, numberofunits, yearbuilt,
         lotsize_acres, lotsize_sf, address_type,
         category, category_detail, residential, building_type, area_tax, zone_tax, possibleSF_tax, stories_tax, category_tax, units_tax, parcel_id, num_livunit, dwellings_per_parcel, land_per_parcel)
