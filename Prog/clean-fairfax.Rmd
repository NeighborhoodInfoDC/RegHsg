...
jur_1 <- jur %>% 
  ...
  #due to very high lot sizes in blackknight - if parcel area is much less than lotsize_sf, use parcel area instead
  mutate(lotsize_sf = case_when(
         (lotsize_sf - parcel_area) > 1000000 ~parcel_area,
         TRUE ~ lotsize_sf
  ...
  mutate(lotsize_sf = case_when(
         is.na(lotsize_sf) ~ jur$area_legal%>%as.double,
         T ~ lotsize_sf
  )) %>% 
    mutate_at(vars(lotsize_sf), ~ replace(., .==0, NA)) %>%
  mutate(lotsize_sf = case_when(
            (is.na(lotsize_sf)|lotsize_sf==0)~jur$area_tax %>% as.double,
            T ~ lotsize_sf
  ))  %>% mutate_at(vars(lotsize_sf), ~ replace(., .==0, NA)) %>%
    mutate(lotsize_sf = case_when(
        is.na(lotsize_sf) ~
         jur$parcel_area %>% as.double(),
        T ~ lotsize_sf)) 
