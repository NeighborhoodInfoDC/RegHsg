...
####then clean up the SSL with leading alphabets

jursquaremutate <- jursquare %>% 
  mutate(square= substr(assessorsparcelnumberapnpin, 1, 4),
         lot= substr(assessorsparcelnumberapnpin, 11, 14),
...
jursquaremutate2 <- jursquaremutate %>% 
  mutate(newparcelid = paste0(square, newsuffix3,lot)) 

jursquaremutate3 <- select( jursquaremutate2, -newsuffix, -newsuffix2, -suflen, -characterdetect, firstcharacter)


######then merge the two dataset together to get the new blackknight dataset
finaljur <- bind_rows(jurrestmutate3, jursquaremutate3) 

#### check if it has the same rows as the raw dataset
stopifnot(nrow(finaljur) == nrow(jurraw))

#CHECK if duplate ids are introduced after the cleaning
dup <- finaljur %>% group_by(newparcelid) %>% filter(n()>1) 
...
jur <- left_join(parcelbaseDC,finaljur, 
                 by = c( "parcelbase_SSL"= "newparcelid" ))

#20112 observation from black knight didn't merge
nonmatchingblackknight <- anti_join(finaljur, parcelbaseDC ,
                 by = c("newparcelid" = "parcelbase_SSL"))

#2512 observations from parcelbase dataset didn't merge
nonmatchingparcel <- anti_join(parcelbaseDC, finaljur, 
                 by = c( "parcelbase_SSL" = "newparcelid"))
