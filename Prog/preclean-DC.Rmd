...
jurpar <- jurraw %>% 
  filter(str_detect(assessorsparcelnumberapnpin, "PAR")) %>% 
  mutate(newparcelid= paste0(substr(assessorsparcelnumberapnpin, 1, 4), substr(assessorsparcelnumberapnpin, 6, 9), substr(assessorsparcelnumberapnpin, 11, 14))
         )
...
jurmutate <- jurrest %>% 
  mutate(square= substr(assessorsparcelnumberapnpin, 1, 4),
         ...
         newsuffix2= str_replace(newsuffix, " ", "-"),
         suflen=str_length(newsuffix2))


 jurmutate2 <-  jurmutate %>% 
        mutate (suffix= str_detect(assessorsparcelnumberapnpin, "[:alpha:]"== 1,1,0),
                newparcelid= ifelse(suffix==0, paste0(square, "----", lot), paste0(square, newsuffix2,lot)))                                                                                    
                  


dup2 <- parcelgeoDC %>% group_by(parcelgeo_SSL) %>% filter(n()>1) 
...
parcelbaseDC <- parcel %>%
            mutate(parcelbase_SSL = paste0(substr(parcelbase_parcelID, 1, 4),       str_pad(parcelbase_suffix,width=4, side="right",pad="-"), substr(parcelbase_parcelID, 9, 12)))

#no dups after cleaning up the parcel base data SSL
dup2 <- parcelbaseDC %>% 
...
nonmatching <- anti_join ( parcelbaseDCraw, parcelgeoDCraw, by=c("parcelbase_parcelID"="parcelgeo_parcelID"))

#19738 observation from black knight didn't merge
nonmatchingblackknight <- anti_join(jurid, parcel ,
                 by = c("new_parcelnumber" = "parcelbase_SSL"))
