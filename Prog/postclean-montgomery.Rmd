...
zjoin <- st_join(jur_sf1, zoning_sf1, join = st_intersects)
...
zjoin_df <- st_set_geometry(zjoin, NULL)
...
paste0(sum(is.na(zjoin_df$validgeo)), " observations didn't join- ",
      round(sum(is.na(zjoin_df$validgeo)) / nrow(zjoin_df) * 100, 2), "%")

if (sum(is.na(zjoin_df$validgeo)) / nrow(zjoin_df) > .05) {
  stop("More than 5% of observations could not be spatially joined.")
}

paste0(sum(is.na(zjoin_df$CODE)), " observations didn't join- ",
      round(sum(is.na(zjoin_df$CODE)) / nrow(zjoin_df) * 100, 2), "%")
```
...
```{r count-lots}
sum(is.na(zjoin_df1$arc_zoning)) - sum(is.na(zjoin_df1$zoning))
```
...
zjoin_df1 %>% 
  ...
  mutate(different_flag = ifelse(zoning != CODE,
                                 1,
                                 0)) %>% 
  summarize(different = sum(different_flag, na.rm = TRUE)) %>% 
  mutate(percent_different = paste0(round(different / nrow(zjoin_df) *100, 2), "%"))
