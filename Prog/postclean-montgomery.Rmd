...
```{r join rockville}
#Sarah - i gain 6 obs here - can you figure out why and/or drop? 

overlap <- st_intersection(rockzoning_sf1) %>% 
  filter(n.overlaps > 1)

ggplot() +
  geom_sf(overlap, mapping = aes()) +
  coord_sf(datum = NA)

rjoin <- st_join(pjoin2, rockzoning_sf1, join = st_intersects)

mult <- rjoin %>% 
  as.data.frame() %>% 
  group_by(propaddress) %>% 
  summarize(zonelist = list(ZONE_)) %>% 
  mutate(unq = map(zonelist, ~ length(unique(.)))) %>% 
  filter(unq > 1)

mult1 <- rjoin %>% 
  as.data.frame() %>% 
  group_by(assessorsparcelnumberapnpin, propaddress, ZONE_) %>%
  summarize(count = n()) %>% 
  mutate(rown = row_number()) %>% 
  filter(rown > 1)

rjoin1 <- rjoin %>% 
  group_by(propaddress, ZONE_) %>% 
  filter(row_number() == 1) %>% 
  rbind(filter(rjoin, is.na(propaddress))) %>% 
  ungroup()
