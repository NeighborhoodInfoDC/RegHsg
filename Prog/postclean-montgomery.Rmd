...
landuse <- pull(prop_testing, LU_CATEGORY) %>% unique()
...
Read file, test geomerty and projection.
...
rjoin <- st_join(zjoin, rockzoning_sf1, join = st_intersects)
...
Repeat this process with Gaithersburg. The Gaithersburg file is not a zoning file, though, so we don't get any more detailed zoning information. This flag just tells us if the property falls in Gaithersburg city limits.

Read the file, check the validity, transform to match BK CRS.
```{r check-g-geo}
...
gjoin <- st_join(rjoin2, gait_sf1, join = st_intersects)

paste0(sum(!is.na(gjoin$CLASSFP)), " observations did join- ",
      round(sum(!is.na(gjoin$CLASSFP)) / nrow(gjoin) * 100, 2), "%")

gjoin2 <- gjoin %>% 
  mutate(gaitflag = if_else(!is.na(CLASSFP), 1, 0))

# test that observations are in gaithers
test <- gjoin2 %>% 
  filter(gaitflag ==1) %>% 
  sample_n(1000)
