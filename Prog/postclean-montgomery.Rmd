...
leftovers <- st_difference(jur_sf1, parcel_sf1)

leftovers2 <- st_disjoint(jur_sf1, parcel_sf1)
...
paste0(sum(is.na(pzjoin_df$validgeo)), " observations didn't join- ",
      round(sum(is.na(pzjoin_df$validgeo)) / nrow(pzjoin_df) * 100, 2), "%")
...
pzjoin_df1 %>% 
  ...
  mutate(percent_different = paste0(round(different / nrow(pzjoin_df) *100, 2), "%"))
...
```{add back those that didn't match}

pzjoin_df2 <- bind_rows(pjoindf1, leftovers)
