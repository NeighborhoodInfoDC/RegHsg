...
sum(is.na(pjoin4$lotsize_sf))
sum(pjoin4$lotsize_sf == 0, na.rm = TRUE)
...
# Biglots <- pjoin_df1 %>%
#   group_by(OBJECTID)%>%
#   summarize_at(vars(zoning,category_detail, LU_CATEGOR, lotsize_sf,lotwasmissing,SHAPE_AREA), list) %>% 
...
#  mutate(shapefirst =as.double(shapefirst),
#         avglot= shapefirst/nprops)
#
#Onelot <- Biglots3 %>%
...
```{r count-lots size}
sum(is.na(pjoin_df$lotsize_sf)) - sum(is.na(pjoin4$lotsize_sf))
```
...
#A dd flag for gaithersburg and rockville
