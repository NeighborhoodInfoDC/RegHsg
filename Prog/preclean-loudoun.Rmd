...
parcel <- parcel %>% 
  ...
  mutate(parcel_id = paste0(raw_parcelid_par)) %>% 
...
The Loudon file does not have address, lat or long -RP
```{r select-prop}
property <- property %>% 
  select(raw_parcelid_prop = LU_PIN,
         lotsize_prop = LU_LEGAL_ACRE,
         numberofunits_prop = LU_HUNITS_EXIST) %>% 
  mutate(parcel_id = paste0(raw_parcelid_prop))
